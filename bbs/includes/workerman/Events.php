...
class Events
{
   ...
   private static function adminLogin($client_id, $data)
   {
		...
		foreach($team_guests AS $idx => $guest){
			if($guest['aid'] == $admin_id){
				...
				$guest_list[] = array('g' => $guest['gid'], 'n' => $guest['fullname'], 'iz' => $guest['ipzone'], 'l' => $guest['lang'], 'au' => $guest['au'], 'mb' => $guest['mb'], 'fr' => "", 're' => $recs);
			}
		}
    ...
    */
	private static function tranferGuest($client_id, $data, $guest_query = 0)
	{
		if($guest_query == 1){//客人请求转接客服(客服离线时)
			...
			//无法分配客服, 返回特殊信息通知客人
			if($support === false){
				self::sendToClient($client_id, array('x' => 6, 'a' => 9, 'i' => ""));
		...
		$dd = array('x' => 6, 'a' => 8, 'g' => $gid, 'n' => $guest['fullname'], 'iz' => $guest['ipzone'], 'au' => $guest['au'], 'l' => $guest['lang'], 'mb' => $guest['mb'], 're' => $recs);
    ...
    */
	private static function guestLogin($client_id, $data)
	{
		...
		//人工服务时, 给客服发送通知: 客人上线
		if(!$is_robot){
			$dd = array('x' => 6, 'a' => 8, 'g' => $gid, 'au' => $authupload, 'n' => $fullname, 'fr' => $fromurl, 'iz' => $ipzone, 'l' => $lang, 'mb' => $mobile, 're' => $recs);
			self::sendToClient($aix, $dd);
		}
