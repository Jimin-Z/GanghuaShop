			...
			switch(data.a){
				...
				case 8: //客服连接验证成功
					...
					$.each(data.gl, function(i, guest){
						guest_create(parseInt(guest.g), guest.n, guest.l, guest.re, parseInt(guest.au), parseInt(guest.mb), guest.iz, guest.fr, 1); //创建已连接的客人, 在创建中输出信息, 参数1客服重新连接
					});
...
function guest_create(gid, n, lang, recs, au, mb, iz, fr, old){
	if(!gid) return;
	...
	if(index_guest > -1 && index_offline > -1){ //表示客人重新上线
		...
		g_online.prepend('<div id="gst_' + gid + '" onclick="openWin(' + gid + ');" class="g" title="' + n + ' (' + iz + ')"><div>' + ((mb == 1)? '<u></u>'  : '') + '<b>0</b></div><s>' + iz + '</s><i>' + n + '</i><p></p></div>');
...
function welive_init(){
	...
	sysinfo.tipTip({content: '1. 按 Ctrl + Alt: 在客服交流区与当前客人小窗口间切换.<br>2. 按 Ctrl + 下箭头 或 Esc键: 关闭当前客人小窗口.<br>3. 按 Ctrl + 上箭头: 展开关闭的客人小窗口.<br>4. 按 Ctrl + 左或右箭头: 在已展开的客人小窗口间切换.<br>5. 客人被踢出或禁言后, 其刷新页面仍可重新进入客服.<br>6. 客人获取上传授权后将会一直保留, 直至被客服解除.<br>7. 组长管理员发送all, admin, guest, robot查询运行数据.', maxWidth:"400px", defaultPosition:"top", delay:300});
	...
	//管理员或组长专有
	if(admin.type != 0){
		$(".set_robot_on").tipTip({content: '1. 每个客服组的无人值守状态均为独立设置, 互不影响.<br>2. 无人值守开启后, 所有本组客服均可离线.<br>3. 无人值守开启后, 新上线的访客将由机器人自动回复, 已经<br>&nbsp;&nbsp;&nbsp;&nbsp;在线的访客仍由原客服提供服务.<br>4. 无人值守开启后, 已登录的客服可接受来自机器人转接人工<br>&nbsp;&nbsp;&nbsp;&nbsp;客服而来的客人.<br>5. 无人值守开启后, 访客留言功能将失效.<br>6. Socket服务重启后, 无人值守状态需要重新设置.'});
	}
