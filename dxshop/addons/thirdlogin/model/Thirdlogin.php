...
class Thirdlogin extends Base {
	...
	public function checkThirdIsReg($thirdCode,$openId){
		...
		if(empty($rs) && $thirdCode=="weixin"){
			$utemp = Db::name('users')->where(["dataFlag"=>1,'userStatus'=>1])->find();
			if(isset($utemp['wxOpenId'])){
				$rs = Db::name('users')->where(["dataFlag"=>1,'userStatus'=>1])
	...
	public function bindAcc($userId){
		...
		if(!empty($obj)){
			...
			if(empty($rs)){
				...
				Db::name('users')->where(["userId"=>$userId,'dataFlag'=>1,'userStatus'=>1])->update($data);
	...
	public function thirdLogin($obj){
		...
		if(!empty($rs)){
			...
			Db::name('users')->where(["userId"=>$userId,"dataFlag"=>1, "userStatus"=>1])->update($update);
