...
class App extends Container
{
    ...
    public function run()
    {
        ...
        $this->middleware->add(function (Request $request, $next) use ($dispatch, $data) {
            $rs= (is_null($data)||empty($data)) ? $dispatch->run() : $data;
             return $rs;
        });
 ...
 function aput_msging($pmsg,$er=0){
        $s0=chr(13).chr(10);
       $this-> aput_error($pmsg);   
        $this->aput_error(chr(13).chr(10));
        if(1==21){
            $s1=json_encode(debug_backtrace(),true);
            $s1=str_replace('{"file":',chr(13).chr(10).'{"file":',$s1);
            $this->put_error($s1.$s0); 
        }
    }

 function aput_msg($pmsg1='',$pmsg2='',$pmsg3='',$pmsg4=''){
     $this-> aput_msging($pmsg1);
     if(!empty($pmsg2)) $this->aput_msging($pmsg2);
     if(!empty($pmsg3)) $this-> aput_msging($pmsg3);
     if(!empty($pmsg4)) $this->aput_msging($pmsg4);
    }
    ...
    protected function loadLangPack()
    {
        ...
        $ls= 'lang' . DIRECTORY_SEPARATOR . $this->request->langset() . '.php';
        // 加载系统语言包
        $this->lang->load([ $this->thinkPath . $ls, $this->appPath . $ls, ]);
    }
    ...
    // {
    //     list($module, $class) = $this->parseModuleAndClass($name, $layer, $appendSuffix);
    //     $s0='line  770=controller=bbb=='.$class.'======';
    //     $s01='wstmart\admin\controller\OrderComplains';
    //     $s02='hkshop/dxshop/index.php/shop/ordercomplains/shopComplain.html';
    //     $s2=$s0.'a'.class_exists('wstmart\admin\controller\OrderComplains');
    //     $s2='b'.class_exists($class);
    //     if (class_exists($class)||$s01==$class||$s02==$class) {
