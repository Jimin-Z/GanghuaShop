...
class Think
{
    ...
    private function parseTemplate($template)
    {
        ...
        if (0 !== strpos($template, '/')) {
            ...
            $controller   = str_replace('order_complains', 'OrderComplains',  $controller);
            if ($controller) {
                $s1=str_replace('.', DIRECTORY_SEPARATOR, $controller) . $depr;
                if ('' == $template) {
                    // 如果模板文件名为空 按照默认规则定位
                    $template = $s1 . $this->getActionTemplate($request);
