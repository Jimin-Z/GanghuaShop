...
class CashDraws extends Base{
		...
		try{
            ...
            if(false != $result){
                ...
                if($cash->commission>0){
                    $commissionLog = '，手续费¥'.$cash->commission;
                }
            	//创建一条流水记录
            	$lm = [];
				...
				$lm['dataId'] = $id;
				$lm['dataSrc'] = 3;
				$lm['remark'] = '提现申请单【'.$cash->cashNo.'】申请提现¥'.$cash->money.$commissionLog.'，实际到账金额¥'.$cash->actualMoney.'。'.(($cash->cashRemarks!='')?"【操作备注】：".$cash->cashRemarks:'');
				$lm['moneyType'] = 0;
				$lm['money'] = $cash->money;
				$lm['payType'] = 0;
                ...
                WSTWeMessage(['CODE'=>'WE_CASH_DRAW_RESULT','userId'=>$targetId,'params'=>$params]);
				Db::commit();
	...
	public function toExport(){
		...
		->setCellValue('D1', '提现账号信息')
		->setCellValue('E1', '持卡人')->setCellValue('F1', '提现金额')
		->setCellValue('G1', '提现时间')->setCellValue('H1', '状态');
		...
		for ($row = 0; $row < count($page); $row++){
			...
			->setCellValue('E'.$i, $page[$row]['accUser'])->setCellValue('F'.$i, '￥'.$page[$row]['money'])
