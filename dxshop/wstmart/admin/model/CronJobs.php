...
class CronJobs extends Base{
	...
	public function autoNotifyGoodsPriceReduction(){
		// -- 取出关注商品表，以商品id为组区分
		$favoriteGoods = Db::name("favorites")->group("goodsId")->field("goodsId, max(currShopPrice) currShopPrice")->select();
		if(empty($favoriteGoods))return;
		$goodsIds = array_column($favoriteGoods, "goodsId");
		// 取出所有关联的商品，
		$goodsRs = Db::name('goods')->field("goodsId,goodsName,shopPrice")
									->whereIn("goodsId", $goodsIds)
									->where(['dataFlag'=>1, 'goodsStatus'=>1, 'isSale'=>1])
									->select();
		// 构造数据，便于获取数据查询
		$goods = [];
		foreach($goodsRs as $v){
			$goods[$v['goodsId']] = $v;
		}
		unset($v);
		// 降价的商品id
		...
		foreach($favoriteGoods as $k=>$v){
			// 商品状态正常 且 当前商品价格 < 收藏时的价格
			if(isset($goods[$v['goodsId']]) && $goods[$v['goodsId']]['shopPrice'] < $v['currShopPrice']){
				array_push($reductionGIds, $v['goodsId']);
			}
		}
		Db::startTrans();
		try{
			// 更新currShopPrice、发送商城消息
			foreach($reductionGIds as $goodsId){
				$goodsItem = $goods[$goodsId];
				// 更新favorites表currShopPrice字段
				Db::name("favorites")->where(['goodsId'=>$goodsId])->update(["currShopPrice"=>$goodsItem['shopPrice']]);
				// 获取用户id，发送商城消息
				$userIds = Db::name("favorites")->where(['goodsId'=>$goodsId])->column("userId");
				//发送消息
				$tpl = WSTMsgTemplates('GOODS_PRICE_REDUCTION');
				if( $tpl['tplContent']!='' && $tpl['status']=='1'){
					$find = ['${GOODS}'];
					$replace = [$goodsItem['goodsName']];
					// 批量发送商城消息
					// 发送用户信息
					$content = str_replace($find,$replace,$tpl['tplContent']);
					$msgJson = ['from'=>1,'dataId'=>$goodsId];
					WSTSendMsg($userIds, $content,  $msgJson);
				}
			}
			Db::commit();
		}catch(\Exception $e){
		 		...
		 		foreach ($rs as $key => $order){
		 			...
		 			if(false !== $rsStatus){
                        ...
                        if($shopMember){
                            ...
                            if($groupId>0){
                                $member = Db::name('shop_members')->where(['userId'=>$order['userId'],'shopId'=>$order['shopId']])->find();
                                if($member['groupId'] != $groupId){
                                    Db::name('shop_members')->where(['userId'=>$order['userId'],'shopId'=>$order['shopId']])->update(['groupId'=>$groupId]);
                                }
                            }
                        }
						//发送一条商家信息
						$tpl = WSTMsgTemplates('ORDER_AUTO_RECEIVE');
		                if( $tpl['tplContent']!='' && $tpl['status']=='1'){
		                    $find = ['${ORDER_NO}'];
		                    $replace = [$order['orderNo']];
		                	$msg = array();
    ...
    public function autoChangeShopStatus(){
        ...
        try{
            foreach($shops as $key => $vo){
                if(WSTStrToTime($vo['expireDate'])<WSTStrToTime(date('Y-m-d'))){
