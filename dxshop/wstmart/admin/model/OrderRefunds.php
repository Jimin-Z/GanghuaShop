...
class OrderRefunds extends Base{
	...
	public function refundPageQuery(){
		...
		$page = Db::name('orders')->alias('o')->join('__SHOPS__ s','o.shopId=s.shopId','left')
			 ...
			 ->where(function ($query){
                 $query->whereor([['orderStatus','in',[-1,-3]],['serviceId','<>',0]]);
             })
	...
	public function complateOrderRefund($obj){
        ...
        try{
            ...
            $params = [];
            $params['REFOUND_RESULT'] = "退款成功";
            $params['REFOUND_TIME'] = date("Y-m-d H:i:s");
            ...
            WSTWeMessage(['CODE'=>'WE_ORDER_REFUND_RESULT','userId'=>$order->userId,'params'=>$params]);
			//如果有钱剩下，那么就退回到商家钱包
	...
	public function toExport(){
		...
		$page = Db::name('orders')->alias('o')->join('__SHOPS__ s','o.shopId=s.shopId','left')
		->join('__USERS__ u','o.userId=u.userId','left')
		->join('__ORDER_REFUNDS__ orf ','o.orderId=orf.orderId and orf.refundStatus in (1,2)')
		->where($where)
