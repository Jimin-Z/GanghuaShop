...
class Reports extends Base{
    ...
    public function topSaleGoodsByPage(){
    	...
    	return Db::table($prefix.'order_goods')
          ...
          ->field('og.goodsId,g.goodsName,g.goodsSn,s.shopId,s.shopName,sum(og.goodsNum) goodsNum,og.goodsImg')
          ...
          ->where('(o.payType=0 or (o.payType=1 and o.isPay=1)) and o.dataFlag=1')
          ->where([['o.orderStatus','>=',0]])
          ->group('og.goodsId,g.goodsName,g.goodsSn,s.shopId,s.shopName,og.goodsImg')
    ...
    public function topShopSalesByPage(){
        ...
        $rs = Db::table($prefix.'shops')
                 ...
                 ->field('s.shopId,s.shopImg,s.shopName,sum(realTotalMoney+scoreMoney) totalMoney,count(o.orderId) orderNum')
                 ->join($prefix.'orders o','s.shopId=o.shopId')
                 ->order('totalMoney desc,orderNum desc')
                 ->whereTime('o.createTime','between',[$start,$end])
                 ->where('(payType=0 or (payType=1 and o.isPay=1)) and o.dataFlag=1')
        ...
        foreach($rs['data'] as $k=>$v){
            $onLineArr = Db::name('orders')
                 ...
                 ->where('payType=1 and isPay=1 and dataFlag=1')
                 ->where([['orderStatus','=',2]])
                 ->where(['shopId'=>$v['shopId']])
                 ->find();
            $rs['data'][$k]['onLinePayMoney'] = (float)$onLineArr['totalRealTotalMoney'];// 在线支付总金额
            $rs['data'][$k]['onLinePayScoreMoney'] = (float)$onLineArr['totalScoreMoney'];// 积分抵扣金额
            $rs['data'][$k]['offLinePayMoney'] = (float)
            Db::name('orders')
                 ->whereTime('createTime','between',[$start,$end])
                 ->where('payType=0 and dataFlag=1')
                 ->where(['shopId'=>$v['shopId']])
    ...
    public function statSales(){
        ...
        $rs = Db::field('left(createTime,10) createTime,orderSrc,sum(realTotalMoney+ scoreMoney) totalMoney')
                ->name('orders')
                ->whereTime('createTime','between',[$start,$end])
        ...
        if(count($rs)>0){
            //取消订单
            $cancelMaps = [];
            $cancelRs = Db::field('left(createTime,10) createTime,sum(realTotalMoney+ scoreMoney) totalMoney')
                ->name('orders')
                ->whereTime('createTime','between',[$start,$end])
                ->where('((payType=0 or (payType=1 and isPay=1)) and dataFlag=1) '.(in_array($payType,[0,1])?" and payType=".$payType:''))
                ->where(['orderStatus'=>-1])
            ...
            //拒收订单
            $rejectMaps = [];
            $rejectRs = Db::field('left(createTime,10) createTime,sum(realTotalMoney+ scoreMoney) totalMoney')
                ->name('orders')
                ->whereTime('createTime','between',[$start,$end])
                ->where('((payType=0 or (payType=1 and isPay=1)) and dataFlag=1) '.(in_array($payType,[0,1])?" and payType=".$payType:''))
                ->where(['orderStatus'=>-3])
            ...
            foreach($days as $v){
                ...
                $rdata['total'][] = $total;
                $rdata['cancelTotal'][] = isset($cancelMaps[$v])?$cancelMaps[$v]:0;
                $rdata['rejectTotal'][] = isset($rejectMaps[$v])?$rejectMaps[$v]:0;
            }
            $rdata['days'] = $days;
        }
    ...
    public function statOrders(){
        ...
        $end = date('Y-m-d 23:59:59',strtotime(input('endDate')));
        $payType = (int)input('payType',-1);
        $rs = Db::field('left(createTime,10) createTime,orderSrc,count(orderId) orderNum')
                ->name('orders')
                ->whereTime('createTime','between',[$start,$end])
                ->where('((payType=0 or (payType=1 and isPay=1)) and dataFlag=1) '.(in_array($payType,[0,1])?" and payType=".$payType:''))
                ->order('createTime asc')
        ...
        if(count($rs)>0){
            //取消订单
            $cancelMaps = [];
            $cancelRs = Db::field('left(createTime,10) createTime,count(orderId) orderNum')
                ->name('orders')
                ->whereTime('createTime','between',[$start,$end])
                ->where('((payType=0 or (payType=1 and isPay=1)) and dataFlag=1) '.(in_array($payType,[0,1])?" and payType=".$payType:''))
                ->where(['orderStatus'=>-1])
                ...
                ->group('left(createTime,10)')->select();
            foreach($cancelRs as $v){
                $cancelMaps[$v['createTime']] = $v['orderNum'];
            }
            //拒收订单
            $rejectMaps = [];
            $rejectRs = Db::field('left(createTime,10) createTime,count(orderId) orderNum')
                ->name('orders')
                ->whereTime('createTime','between',[$start,$end])
                ->where('((payType=0 or (payType=1 and isPay=1)) and dataFlag=1) '.(in_array($payType,[0,1])?" and payType=".$payType:''))
                ->where(['orderStatus'=>-3])
                ...
                ->group('left(createTime,10)')->select();
            foreach($rejectRs as $v){
                $rejectMaps[$v['createTime']] = $v['orderNum'];
            }
    ...
    public function toExportSaleGoods(){
        ...
        $rs = Db::table($prefix.'order_goods')
          ...
          ->order('goodsNum','desc')
          ->whereTime('o.createTime','between',[$start,$end])
          ->where('(o.payType=0 or (o.payType=1 and o.isPay=1)) and o.dataFlag=1')->group('og.goodsId,g.goodsName,g.goodsSn,s.shopId,s.shopName,og.goodsImg')
          ->select();
        require Env::get('root_path') . 'extend/phpexcel/PHPExcel/IOFactory.php';
        $objPHPExcel = new \PHPExcel();
        // 设置excel文档的属性
