...
class Settlements extends Base{
    ...
    public function pageShopQuery(){
    	...
    	$shopName = input('shopName');
        $para=getParameter();
        put_msg('pageShopQuery line 74',$para);
        $where=[];
        ...
        if(isset($para['startDate'])&&(!empty($para['startDate']) )){
          $where[] = ['createTime','>=',$para['startDate']];
        }
        if(isset($para['endDate'])&&(!empty($para['endDate']) )){
          $where[] = ['createTime','<=',$para['endDate']];
        }
        put_msg( $where);
