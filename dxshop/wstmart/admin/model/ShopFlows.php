...
class ShopFlows extends Base{
    ...
    public function del(){
		...
		try{
	        ...
	        if(false !== $result){
                ...
                // 检查入驻商家的applyStep等于删除的流程id，要将该字段的值变为平台流程的第一步，否则用户点击入驻进度查询会出现重定向的bug
                $minFlowId = $this->where(['isShow'=>1,'dataFlag'=>1])->min('flowId');
                $shopIds = Db::name('shops')->where(['applyStep'=>$id])->column('shopId');
                Db::name('shops')->where([['shopId','in',$shopIds]])->update(['applyStep'=>$minFlowId]);
	        	Db::commit();
    ...
    public function editiIsShow(){
        ...
        if(false !== $result){
            if($isShow==0){
                // 检查入驻商家的applyStep等于隐藏的流程id，要将该字段的值变为平台流程的第一步，否则用户点击入驻进度查询会出现重定向的bug
                $minFlowId = $this->where(['isShow'=>1,'dataFlag'=>1])->min('flowId');
                $shopIds = Db::name('shops')->where(['applyStep'=>$id])->column('shopId');
                Db::name('shops')->where([['shopId','in',$shopIds]])->update(['applyStep'=>$minFlowId]);
            }
    ...
    public function fieldPageQuery(){
        ...
        $dataType = input("dataType",-1);
        $fieldTitle = input("fieldTitle");
        $isRequire = (int)input("isRequire",-1);
        $fieldType = input("fieldType",-1);
        $where = [];
        $where[] = ['dataFlag','=',1];
        $where[] = ['flowId','=',$flowId];
