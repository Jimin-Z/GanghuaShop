...
class Shops extends Base{
    ...
    public function del(){
        ...
        try{
	        ...
	        // 店铺申请表的图片标记为删除
            // 2022-07-03屏蔽，如果删除以下这几个入驻字段，这里就会报错
    ...
    public function getById($id){
        ...
        //獲取經營范圍
        $goodscats = Db::name('cat_shops cs')->join('goods_cats gc','cs.catId=gc.catId','inner')
                    ->field('cs.*,gc.catName,gc.commissionRate,cs.commissionRate shopCommissionRate')
                    ->where(['cs.shopId'=>$id])
                    ->order('gc.catSort asc')
                    ->select();
        $shop['catshops'] = [];

        foreach ($goodscats as $k=>$v){
            $goodscats[$k]['commissionRate'] = ($v['commissionRate']>0)?$v['commissionRate']:0;
            if($shop['commissionType']==1){
                $goodscats[$k]['shopCommissionRate'] = ($v['shopCommissionRate']>0)?$v['shopCommissionRate']:0;
            }else{
                $goodscats[$k]['shopCommissionRate'] = $goodscats[$k]['commissionRate'];
            }
            
            $shop['catshops'][$v['catId']] = true;
        }

        $shop['vipCatshops'] = $goodscats;
		return $shop;
    }
	...
	public function handleApply(){
        ...
        $shopName = input("shopName");
        $tsp = Db::name("shops")->where([["dataFlag",'=',1],['shopName','=',$shopName],['shopId','<>',$shopId]])->find();
        if(!empty($tsp)){
        ...
        try{
            ...
            $commissionType = (int)input("commissionType");
            $shopsData['commissionType'] = ($commissionType>0)?1:0;
            ...
            if($goodsCats){
                ...
                foreach ($goodsCats as $key =>$v){
                    if((int)$v>0){
                        $commissionRate = 0;
                        if($commissionType==1){
                            $commissionRate = (float)input("commissionRate_".$v);
                            if($commissionRate>90 || $commissionRate<0){
                                return WSTReturn("请输入正确的佣金比例");
                            }
                        }
                        Db::name('cat_shops')->insert(['shopId'=>$shopId,'catId'=>$v,'commissionRate'=>$commissionRate]);
                    }
                }
            }
		    //认证类型
	        ...
	        if($data['applyStatus']==2){
                ...
                //修改店铺截止日期
                Db::name('shops')->where(['shopId'=>$shopId])->update(['expireDate'=>date("Y-m-d",strtotime("-1 days",strtotime("+1 years")))]);
	        }
	...
	public function add(){
        ...
        try{
        	...
        	if($isNewUser==1){
		    	...
		    	if($shopUser['userType']>0 && $shopUser['userType']!=1)return WSTReturn($shopUser['userType'].'所关联账号已关联其他角色类型，不能关联店铺');
	...
	*/
	...
	public function searchQuery(){
		$tradeId = (int)input('post.tradeId');
		if($tradeId<=0)return [];
		$key = input('post.key');
		$where = [];
		$where[] = ['dataFlag','=',1];
		...
		return $this->where($where)->field('shopName,shopId,shopSn')->select();
	}
