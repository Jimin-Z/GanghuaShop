...
class Users extends Base{
    ...
    public function del(){
	    ...
	    try{
            //检测是否店铺账号，是的话禁止删除
            $chk = Db::name('shops')->where(['userId'=>$id,'dataFlag'=>1])->count();
            if($chk>0)return WSTReturn('不允许直接删除店铺负责人，请先删除店铺再删除该账号');
            $chk = Db::name('suppliers')->where(['userId'=>$id,'dataFlag'=>1])->count();
            if($chk>0)return WSTReturn('不允许直接删除供应商负责人，请先删除供应商再删除该账号');
		    $data = [];
			$data['dataFlag'] = -1;
	...
	*/
    ...
    public function cancelIllegal(){
        ...
        $id = (int)input('post.id');
        if($illegalRemarks=='')return WSTReturn("请输入不通过原因");
        //判断商品状态
        $rs = Db::name("users")->where(["userId"=>$id,"userStatus"=>-1])->find();
        Db::startTrans();
        try{
            $res = Db::name("users")->where('userId',$id)->update(['userStatus'=>1,'cancelRemarks'=>$illegalRemarks]);
            if($res!==false){
                //发送商城信息
                $tpl = WSTMsgTemplates('USER_CANCLE_REJECT');
                if($tpl['tplContent']!='' && $tpl['status']=='1'){
                    $find = ['${LOGIN_NAME}','${REASON}'];
                    $replace = [$rs['loginName'],$illegalRemarks];
                    WSTSendMsg($id,str_replace($find,$replace,$tpl['tplContent']));
                }
                Db::commit();
                return WSTReturn('操作成功',1);
            }
	...
	public function toExport(){
        ...
        for ($row = 0; $row < count($rs); $row++){
            ...
            ->setCellValue('Q'.$i, ($rs[$row]['userStatus']==1)?"启用":"停用");
            $totalRow++;
        }
