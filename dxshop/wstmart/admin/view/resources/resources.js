...
function initGrid(p){
	mmg = layui.table;
	mmg.render({elem: '#mmg',id:'dataTable',url:WST.U('admin/resources/pageQuery',{keyword:$("#key").val()}),cellMinWidth: 80,height: WST.initGridHeight(),skin: 'line',even: true,limit:20,page:{curr:p},
		cols: [[
            {title:'资源文件', field:'resPath', templet: function(item){
				if(item['resType']==0){
            ...
            {title:'资源类型', field:'resType', templet: function(item){
				var text = "";
				switch(item['resType']){
					case 0:
						text = "图片";
            ...
            {title:'上传者', field:'userName' ,templet: function(item){
               if(item['fromType']==1){
            ...
            {title:'文件大小(M)', field:'resSize' },
            {title:'状态', field:'isUse' ,templet: function(item){
               return (item['isUse']==1)?"<span class='statu-yes'><i class='fa fa-check-circle'></i> 有效</span>":"<span class='statu-no'><i class='fa fa-ban'></i> 无效</span>";
            }},
...
function loadGrid(p){
	p=(p<=1)?1:p;
	var params = {};
	params.keyword = $('#key').val();
	params.isUse = $('#isUse').val();
	params.resType = $('#resType').val();
