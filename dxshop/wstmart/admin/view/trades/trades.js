...
function initGrid(){
	grid = $('#maingrid').WSTGridTree({
        ...
        rownumbers:true,
        columns: [
	        { display: '行业名称', name: 'tradeName', id:'tradeId', align: 'left',isSort: false,render: function (item)
                {
	        ...
	        { display: '行业名缩写', width: 250,name: 'simpleName', id:'tradeId', align: 'left',isSort: false,render: function (item)
                {
            ...
            { display: '是否显示', width: 120, name: 'isShow',isSort: false,
            ...
            { display: '排序号', name: 'tradeSort',width: 100,isSort: false,render: function (item)
                {
            ...
            { display: '类目费用', width: 150, name: 'tradeFee',isSort: false},
...
function toEdit(pid,id){
    ...
    editBox = WST.open({type:2,title:false,content:WST.U('admin/trades/toEdit','id='+id+'&pid='+pid),closeBtn:0,area: [w+'px', h+'px'],offset:['0px','0px'],end:function(){
    	loadGrid(0);
    }})
}
