...
class Goods extends Base{
    ...
    public function auditByPage($sId=0){
		...
		$rs = $this->where($where)
			->field('goodsId,goodsName,goodsImg,goodsType,goodsSn,isSale,isBest,isHot,isNew,isRecom,goodsStock,saleNum,shopPrice,isSpec,collectNum')
			->order('saleTime', 'desc')
    ...
    public function storeByPage($sId=0){
		...
		$rs = $this->where($where)
		    ->where('goodsStatus','<>',-1)
			->field('goodsId,goodsName,goodsImg,goodsType,goodsSn,isSale,isBest,isHot,isNew,isRecom,goodsStock,saleNum,shopPrice,isSpec,collectNum,canSaleStatus')
			->order('saleTime', 'desc')
			->paginate($pagesize)->toArray();
        foreach ($rs['data'] as $key => $v){
			$rs['data'][$key]['verfiycode'] =  WSTShopEncrypt($shopId);
    ...
    public function illegalByPage($sId=0){
        ...
        $rs = $this->where($where)
            ->field('goodsId,goodsName,goodsImg,goodsType,goodsSn,isSale,isBest,isHot,isNew,isRecom,illegalRemarks,goodsStock,saleNum,shopPrice,isSpec,collectNum')
            ->order('saleTime', 'desc')
	...
	public function changeSale($sId=0){
		...
		if($isSale==1){
			//0.核对店铺状态
	 		...
	 		$cnt = Db::name("goods")->where([['goodsId','in',$ids],['shopId','=',$shopId],['dataFlag','=',1],['canSaleStatus','=',0]])->count();
	 		if($cnt>0){
	...
	public function shopGoods($shopId){
		...
		->field('g.goodsId,g.goodsName,g.goodsImg,g.shopPrice,g.marketPrice,g.saleNum,g.appraiseNum,g.goodsStock,g.isFreeShipping,gallery,gs.totalScore,gs.totalUsers')
		->orderRaw($order)
	...
	public function add($sId=0){
		...
		$goodsCatId = (int)$data['goodsCatId'];
		$childCnt = Db::name("goods_cats")->where(['parentId'=>$goodsCatId,'dataFlag'=>1])->count();
		if($childCnt>0){
        ...
        try{
            if(count($memberReduceMoneyArr)>0 && $specsIds==''){
                ...
                if(!$reduceFlag)return WSTReturn("会员立减价格不能大于或等于店铺价格");
            }
        	//对图片域名进行处理
        	...
        	//保存插件数据钩子
        	hook('beforeEditGoods',['data'=>&$data]);
    	        ...
    	        if($data['goodsType']==0 && $specsIds!=''){
		    		...
		    		foreach ($specsIds as $v){
                        ...
                        if(count($memberReduceMoneyArr)>0) {
                            ...
                            if (!$reduceFlag) return WSTReturn("会员立减价格不能大于或等于多规格价格");
                        }
	...
	public function getById($goodsId,$sId=0){
		...
		if(!empty($rs)){
			...
			$specs = Db::name('spec_cats')->alias('gc')->join('__SPEC_ITEMS__ sit','gc.catId=sit.catId','inner')
			                      ->where(['sit.goodsId'=>$goodsId,'gc.isShow'=>1,'sit.dataFlag'=>1,'gc.dataFlag'=>1])
			                      ->field('gc.isAllowImg,sit.catId,sit.itemId,sit.itemName,sit.itemImg')
			                      ->order('gc.isAllowImg desc,gc.catSort asc,gc.catId asc,sit.itemId asc')->select();
			$spec0 = [];
			$spec1 = [];
			foreach ($specs as $key =>$v){
				if($v['isAllowImg']==1){
                    if($v['itemImg']!=""){
                        $v['itemImgThumb'] = WSTImg($v['itemImg'],1);
                    }
					$spec0[] = $v;
				}else{
					$spec1[] = $v;
