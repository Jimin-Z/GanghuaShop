...
class GoodsAppraises extends Base{
	public function queryByPage($sId=0){
		...
		$model = model('goods');
		$rs = $model->alias('g')
					  ->field('u.userPhoto,u.loginName, g.goodsId,g.goodsImg,g.goodsName,ga.shopReply,ga.id gaId,ga.replyTime,ga.goodsScore,ga.serviceScore,ga.timeScore,ga.content,ga.images,u.loginName,ga.isReport')
	...
	*/
     ...
     * 商家举报评价
     */
    public function shopReport($sId=0){
        $shopId = ($sId==0)?(int)session('WST_USER.shopId'):$sId;
        $reportContent = input('reportContent');
        if($reportContent=='')return WSTReturn('举报内容不能为空');
        $id = (int)input('id');
        $data = [];
        $data['isReport'] = 1;
        $data['reportContent'] = $reportContent;
        $data['reportTime'] = date('Y-m-d H:i:s');
        $rs = $this->where(['id'=>$id,'shopId'=>$shopId])->update($data);
        if($rs !== false){
            return WSTReturn('举报成功',1);
        }
        return WSTReturn('举报失败');
