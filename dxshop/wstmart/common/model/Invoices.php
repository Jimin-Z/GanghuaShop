...
class Invoices extends Base{
	...
	*/
	public function pageQuery($limit=0,$uId=0){
		...
		return $this->where(['userId'=>$userId,'dataFlag'=>1])->where($where)->limit($limit)->select()->toArray();
	}
	/**
	* 新增
	*/
	...
	*/
	public function getInviceInfo($id,$uId=0){
        ...
        $userId = ($uId==0)?(int)session('WST_USER.userId'):$uId;
        $rs = Db::name('invoices')->where(['id'=>$id,'userId'=>$userId,'dataFlag'=>1])->find();
        if(empty($rs))return [];
        WSTUnset($rs,'userId,dataFlag,createTime');
