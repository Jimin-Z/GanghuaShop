...
class OrderServices extends Base{
    ...
    public function crontab(){
        ...
        foreach($rs as $k=>$v){
            ...
            $shopId = $this->alias("os")
                           ...
                           ->where(["os.id"=>$v['id']])
                           ->value("o.shopId");
            $this->sendMessage(0, $shopId, $v['id'], $logContent);
        }
        unset($k, $v);
        $this->whereIn('id',$sIds)->setField('isClose', 1);
        Db::name('log_services')->insertAll($logs);
    ...
    public function shopSend($sId=0){
        ...
        try{
            ...
            if($data['shopExpressType']==0){
                $logContent = "商家发货，无需物流，当面交易";
            }else{
                $expressName = $this->getExpressName($data['shopExpressId']);
                $logContent = "商家发货，物流公司：".$expressName."，物流单号：".$data['shopExpressNo'];
            }
            // 写入日志
            $log = [
    ...
    public function getDetail($type=0, $uId=0, $sId=0){
        ...
        $has = $this->checkOrderService($type, $userId, $shopId);
        if(!$has)return WSTReturn('售后单不存在');
        // 查询
        $rs = $this->alias('s')
                   ->join('orders o','o.orderId=s.orderId','inner')
                   ->where($where)
                   ->field('s.*,o.orderNo,o.shopId')
                   ->find();
        if(!empty($rs)){
            // 查询服务单下的商品信息
            ...
            if($rs['isUserSendGoods']==1 && $rs['expressType']==1 ){
                $rs['expressName'] = $this->getExpressName($rs['expressId']);
            }
            // 商家物流公司
            if($rs['isShopSend']==1 && $rs['shopExpressType']==1 ){
                $rs['shopExpressName'] = $this->getExpressName($rs['shopExpressId']);
            }
            // 若为商家则查询上一次填写的地址信息
            if($type==1){
     ...
     * 根据店铺快递id或者快递名称
     */
    private function getExpressName($expressId){
        return Db::name("shop_express")->alias("se")
                                ->join('__EXPRESS__ e','se.expressId=e.expressId','inner')
    ...
    public function pageQuery($type=0, $uId=0, $sId=0){
        ...
        $rs = $this->alias('os')
                   ...
                   ->join('users u','o.userId=u.userId','inner')
                   ->where($where)
                   ->where($where2)
                   ->field('o.orderNo,os.*,u.loginName')
                   ->order($order)
                   ->paginate(input('pagesize/d',20))
                   ->toArray();
        if(!empty($rs['data'])){
            // 查询售后单下的商品
            foreach($rs['data'] as $k=>$v){
                ...
                // 申请原因
                $rs['data'][$k]['serviceTypeText'] = $this->getServiceTypeText($v['serviceType']);
            }
        }
        return $rs;
    }
        ...
        try{
           ...
           $data['shopReceiveExpireTime'] = date('Y-m-d H:i:s', time()+$shopReceiveDays*24*60*60);
            ...
            if($data['expressType']==0){
                $logContent = "用户退货，无需物流，当面交易";
            }else{
                $expressName = $this->getExpressName($data['expressId']);
                $logContent = "用户退货，物流公司：".$expressName."，物流单号：".$data['expressNo'];
            }
            // 写入日志
            $log = [
                'logTime'=>date('Y-m-d H:i:s'),
    ...
    public function commit($uId=0){
        ...
        if($goodsType==1 && $goodsServiceType!=1){
            return WSTReturn("虚拟商品的服务类型只能为退款");
        }
        ...
        if($has['orderCode']=='combination' &&  isset($has['extraJson'])){
            ...
            if($extraJson['combineType']==1){
                ...
                if(($pendingSwitchCount+$pendingRefundCount)>0){
                    // 【商品组合-固定搭配】
                    return WSTReturn("当前订单存在未完成的售后申请");
                }
                // 存在已完成的“退款”售后单
