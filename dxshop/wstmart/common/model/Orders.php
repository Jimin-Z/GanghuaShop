...
class Orders extends Base{
	...
	public function quickSubmit($orderSrc = 0, $uId=0){
        ...
        if(!WSTCheckPayType($payType)){
            return WSTReturn('下单失败，请选择支付方式');
        }
		//计算最大可用积分
		...
		try{
			...
			if($shopOrder['goodsMoney']>999999999)return WSTReturn("您的订单金额过大，请分开多次下单!");
			//给用户分配卡券
			...
			if(false !== $result){
				...
				// ------- 220621增加记录商品货号、商品编号
				$orderGoods['goodsSn'] = $goods['goodsSn'];
				$orderGoods['productNo'] = $goods['productNo'];
				...
				$orderGoods['commissionRate'] = WSTGoodsCommissionRate($goods['goodsCatId'],$shopId,1);
	...
	public function submit($orderSrc = 0, $uId=0){
		...
		if($carts['goodsTotalMoney']>999999999)return WSTReturn("您的订单金额过大，请分开多次下单!");
		...
		if($deliverType==0){
            ...
            if(!WSTCheckUserAddress($tmp[2])){
                return WSTReturn('下单失败，下单地址不在配送范围内');
            }
			foreach ($tmp as $vv){
				if($vv=='')continue;
			...
			$userInfo = model("common/users")->getFieldsById($userId, ["loginName", "userPhone"]);
			$address = [
				"userAddress"=>"用户选择自提",
		...
		try{
			...
			foreach ($carts['carts'] as $ckey =>$shopOrder){
				...
				if($deliverType==1){//自提
					...
					$order['storeType'] = 1;
					// 获取预留手机号
					$selfTakeUserPhone = input("userPhone_".$shopId);
					if($selfTakeUserPhone==""){
						return WSTReturn("请填写预留手机号");
					}
					if(!WSTIsPhone($selfTakeUserPhone)){
						return WSTReturn("无效的预留手机号");
					}
					$order["userPhone"] = $selfTakeUserPhone;
				}
				$order['totalMoney'] = $order['goodsMoney']+$order['deliverMoney'];
                //积分支付-计算分配积分和金额
	...
	public function userOrdersByPage($orderStatus, $isAppraise = -1, $uId=0){
		...
		$page = $this->alias('o')->join('__SHOPS__ s','o.shopId=s.shopId','left')
		             ...
		             ->field('o.afterSaleEndTime,o.receiveTime,o.orderRemarks,o.noticeDeliver,o.orderId,o.orderNo,s.shopName,s.shopId,s.shopQQ,s.shopWangWang,o.goodsMoney,o.totalMoney,o.realTotalMoney,
		              o.orderStatus,o.deliverType,deliverMoney,o.isPay,payType,payFrom,needPay,isAppraise,o.isRefund,orderSrc,o.createTime,o.useScore,oc.complainId,orf.id refundId,o.orderCode')
	...
	/**
	 * 获取商家订单
	 * $type 1:待发货订单 2：待自提订单
	 */
	public function shopOrdersByPage($orderStatus, $sId=0, $type=0){
		...
		$w1=($refund > 0) ? '' :' and orf.refundStatus=0';
        $page = $page->join('__ORDER_REFUNDS__ orf','orf.orderId=o.orderId'.$w1,'left');
  		$page = $page->field('o.userId,o.orderRemarks,o.noticeDeliver,o.orderId,orderNo,goodsMoney,totalMoney,realTotalMoney,orderStatus,deliverType,deliverMoney,isRefund,o.deliverType deliverTypes
		              ,payType,payFrom,userAddress,isPay,isAppraise,userName,userPhone,orderSrc,o.createTime,orf.id refundId,orf.refundStatus,o.orderCode,o.orderShopRemarks')
			  ->order('o.createTime', 'desc')
	    ...
	    if(count($page['data'])>0){
	    	 ...
	    	 foreach ($goods as $v){
	    	 	$v['goodsName'] = WSTStripTags($v['goodsName']);
	    	 	$v['goodsSpecNames'] = str_replace('@@_@@','、',$v['goodsSpecNames']);
	    	 	$v["goodsImg"]=addPath($v["goodsImg"]);
	    	 	$goodsMap[$v['orderId']][] = $v;
	    	 }
	    	 foreach ($page['data'] as $key => $v){
	    	 	 $page['data'][$key]['orderCodeTitle'] = WSTOrderModule($v['orderCode']);
                 $page['data'][$key]['list'] =(isset($goodsMap[$v['orderId']])) ? $goodsMap[$v['orderId']] : [];
	    	 	 $page['data'][$key]['payTypeName'] = WSTLangPayType($v['payType']);
	    	 	 $page['data'][$key]['deliverTypeName'] = WSTLangDeliverType($v['deliverType']==1);
	...
	public function deliver($uId=0, $sId=0){
        ...
        $userId = ($uId==0)?(int)session('WST_USER.userId'):$uId;
        $deliverType = (int)input('post.deliverType');
        $order = $this->where(['shopId'=>$shopId,'orderId'=>$orderId,'orderStatus'=>0])->field('orderId,orderNo,userId,deliverType,userPhone,userName,realTotalMoney')->find();
		...
		try{
	        ...
	        if(WSTDatas('ADS_TYPE',3)!='' || WSTDatas('ADS_TYPE',4)!=''){
		        ...
		        if($expressId>0){
                    ...
                    $express = Db::name("express e")->join('shop_express se','se.expressId=e.expressId')
                                ->where(['se.id'=>$expressId])
                                ->field("expressName")
                                ->find();
                ...
                $params['USER_NAME'] = $order['userName'];
                $params['PHONENO'] = $order['userPhone'];
                $params['DELIVERY_TIME'] = date("Y-m-d H:i:s");
			    ...
			    //小程序模板
		        WSTWeMessage(['CODE'=>'WE_ORDER_DELIVERY','userId'=>$order['userId'],'URL'=>'pages/users/orders/orders','params'=>$params]);
	...
	public function getByView($orderId, $uId=0){
		...
		//获取订单商品
		$orders['goods'] = Db::name('order_goods')->alias('og')->join('__GOODS__ g','g.goodsId=og.goodsId','left')->where('orderId',$orderId)->field('og.*')->order('id asc')->select();
		foreach ($orders['goods'] as $key => $v) {
			...
			// 快递信息
			$expressInfo = "";
			if($orders['deliverType']==0){
				$expressInfo = Db::name('order_express')
										->alias('oe')
										->join('shop_express se','se.id=oe.expressId')
										->join('express e','e.expressId=se.expressId')
										->where([['oe.orderId','=',$orderId],['oe.isExpress','=',1]])
										->where("find_in_set (".$v['id'].",orderGoodsId)")
										->find();
			}
			$orders['goods'][$key]['expressInfo'] = $expressInfo;
		}
        ...
        $orders["expressData"] = [];
        if($orders['deliverType']==0){
            $orders["expressData"] = Db::name('order_express')
                                    ->alias('oe')
                                    ->join('__SHOP_EXPRESS__ se','se.id=oe.expressId')
                                    ->join('__EXPRESS__ e','e.expressId=se.expressId')
                                    ->where([['oe.orderId','=',$orderId],['oe.isExpress','=',1]])
                                    ->field('e.expressName,oe.expressNo')
                                    ->select();
        }
        //格式化发票信息
	...
	*/
		...
		try{
			...
			if($needPay>0 && false != $rs){
				foreach ($orders as $key =>$v){
					...
					if($v['orderType']==0 && $v['deliverType']==1){
				        ...
				        if( $tpl['tplContent']!='' && $tpl['status']=='1'){
				            ...
				            $params = ['tpl'=>$tpl,'params'=>['MALL_NAME'=>WSTConf('CONF.mallName'),'ORDER_CODE'=>$splieVerificationCode,'SHOP_NAME'=>$storeName,'SHOP_ADDRESS'=>$storeAddress]];
				            put_msg('order 1964');put_msg($params);
	...
	public function toExport(){
		...
		if($name=='RefundOrder'){
            ...
            $page = $page->join('__ORDER_REFUNDS__ orf','orf.orderId=o.orderId and orf.refundStatus=0','left');
        }
		...
		$objRow = $objPHPExcel->getActiveSheet()->getStyle('A1:Y1');
		...
		->setCellValue('F1', '支付方式')->setCellValue('G1', '支付来源')->setCellValue('H1', '配送方式')->setCellValue('I1', '发票状态')->setCellValue('J1', '发票信息')

		->setCellValue('K1', '订单商品')
		->setCellValue('L1', '商品货号')
		->setCellValue('M1', '商品价格')
		->setCellValue('N1', '数量')
		->setCellValue('O1', '物流信息')

		->setCellValue('P1', '订单总金额')
		->setCellValue('Q1', '运费')
		->setCellValue('R1', '实付金额')
		->setCellValue('S1', '下单时间')
		->setCellValue('T1', '付款时间')
		->setCellValue('U1', '发货时间')
		->setCellValue('V1', '收货时间')
		->setCellValue('W1', '取消/拒收原因')
		->setCellValue('X1', '是否退款')
		->setCellValue('Y1', '用户留言');
		...
		for ($row = 0; $row < count($page); $row++){
			...
			$i2 = $i3 = $i;
			$i = $i+(1*$goodsn)-1;
			$objPHPExcel->getActiveSheet()->mergeCells('A'.$i2.':A'.$i)->mergeCells('B'.$i2.':B'.$i)
			->mergeCells('C'.$i2.':C'.$i)->mergeCells('D'.$i2.':D'.$i)->mergeCells('E'.$i2.':E'.$i)->mergeCells('F'.$i2.':F'.$i)
			->mergeCells('G'.$i2.':G'.$i)->mergeCells('H'.$i2.':H'.$i)->mergeCells('I'.$i2.':I'.$i)->mergeCells('J'.$i2.':J'.$i)
			// K L M N O
			->mergeCells('P'.$i2.':P'.$i)->mergeCells('Q'.$i2.':Q'.$i)
			->mergeCells('R'.$i2.':R'.$i)->mergeCells('S'.$i2.':S'.$i)->mergeCells('T'.$i2.':T'.$i)->mergeCells('U'.$i2.':U'.$i)
			->mergeCells('V'.$i2.':V'.$i)->mergeCells('W'.$i2.':W'.$i)->mergeCells('X'.$i2.':X'.$i)->mergeCells('Y'.$i2.':Y'.$i);
			...
			->setCellValue('E'.$i2, $page[$row]['userPhone'])->setCellValue('F'.$i2, $page[$row]['payTypeName'])
			->setCellValue('G'.$i2, ($page[$row]['payFrom'])?WSTLangPayFrom($page[$row]['payFrom']):'')
			->setCellValue('H'.$i2, $page[$row]['deliverType'])->setCellValue('I'.$i2, $page[$row]['isMakeInvoice'])
			->setCellValue('J'.$i2, $page[$row]['invoiceArr'])
			// K L M N O
			->setCellValue('P'.$i2, $page[$row]['totalMoney'])
			->setCellValue('Q'.$i2, $page[$row]['deliverMoney'])->setCellValue('R'.$i2, $page[$row]['realTotalMoney'])
			->setCellValue('S'.$i2, $page[$row]['createTime'])->setCellValue('T'.$i2, $page[$row]['payTime'])
			->setCellValue('U'.$i2, $page[$row]['deliveryTime'])->setCellValue('V'.$i2, $page[$row]['receiveTime'])
			->setCellValue('W'.$i2, $page[$row]['logContent'])
			->setCellValue('X'.$i2, ($page[$row]['isRefund']==1)?'是':'')
			->setCellValue('Y'.$i2, $page[$row]['orderRemarks'])
			;
			...
			for ($row2 = 0; $row2 < $goodsn; $row2++){
				$expressInfo = "";
                $tmpGItem = $page[$row]['goods'][$row2];
                // 快递信息
                if($tmpOrders['deliverType']==0){
                    $expressInfo = Db::name('order_express')
                                            ->alias('oe')
											->join('shop_express se','se.id=oe.expressId')
                                            ->join('express e','e.expressId=se.expressId')
                                            ->where([['oe.orderId','=',$tmpOrders['orderId']],['oe.isExpress','=',1]])
                                            ->where("find_in_set (".$tmpGItem['id'].",orderGoodsId)")
                                            ->find();
                    $expressInfo = $expressInfo['expressName'].$expressInfo['expressNo'];
                }
				$objPHPExcel->getActiveSheet()->setCellValue('K'.$i3, (($tmpGItem['goodsCode']=='gift')?'【赠品】':'').$tmpGItem['goodsName'].(($tmpGItem['goodsSpecNames']!='')?'【'.$tmpGItem['goodsSpecNames'].'】':''))
				->setCellValue('L'.$i3, " ".$tmpGItem['productNo'])
				->setCellValue('M'.$i3, $tmpGItem['goodsPrice'])
				->setCellValue('N'.$i3, $tmpGItem['goodsNum'])
				->setCellValue('O'.$i3, $expressInfo);
	    ...
	    $objPHPExcel->getActiveSheet()->getStyle('A1:Y'.$totalRow)->applyFromArray(array(
				'borders' => array (
						'allborders' => array (
	...
	public function payByWallet($uId=0,$type=0){
		...
		try{
            ...
            foreach ($orders as $order) {
            	...
            	if($order['orderType']==0 && $order->deliverType==1)$order->verificationCode = WSTOrderVerificationCode($order->shopId);
            	$result = $order->save();
                if(false != $result){

                	if($order['orderType']==0 && $order['deliverType']==1){
						//自提订单（已支付）发送核验码
						$tpl = WSTMsgTemplates('PHONE_USER_ORDER_VERIFICATCODE');
				        if( $tpl['tplContent']!='' && $tpl['status']=='1'){
				        	$userPhone = $order->userPhone;;
	...
	*/
	...
	public function orderVerificat($shopId,$storeId=0){
		...
		if(empty($order)){
            ...
            $storeName = '';
            if($storeId>0){
			    $storeName = Db::name("stores")->where(['storeId'=>$storeId])->value('storeName');
            }else{
                $storeName = Db::name("shops")->where(['shopId'=>$shopId])->value('shopName');
            }
			$orderId = $order['orderId'];
			$userId = $order['userId'];
			Db::startTrans();
		    try{
				...
				if(false != $result){
					//确认收货后执行钩子
					$goodss = Db::name('order_goods')->where('orderId',$orderId)->field('goodsId,goodsNum,goodsSpecId,goodsName,goodsSpecNames')->select();
					$gnames = [];
					foreach($goodss as $key =>$v){
                        $goodsSpecNames = $v['goodsSpecNames']?('【'.str_replace("@@_@@","|",$v['goodsSpecNames']).'】'):'';
                        $gnames[] = $v['goodsName'].$goodsSpecNames;
						Db::name('goods')->where('goodsId',$v['goodsId'])->update([
                            'saleNum'=>Db::raw('saleNum+'.$v['goodsNum'])
                        ]);
						if($v['goodsSpecId']>0){
                    ...
                    $params = [];
                    $params['ORDER_NO'] = $order['orderNo'];
                    $params['GOODS'] = implode("；",$gnames);
                    $params['SHOP_NAME'] = $storeName;
                    $params['VERIFICAT_TIME'] = date('Y-m-d H:i:s');
                    //小程序模板
                    WSTWeMessage(['CODE'=>'WE_USER_ORDER_VERIFICAT','userId'=>$order['userId'],'URL'=>'pages/users/orders/orders','params'=>$params]);
	...
	public function toDeliverTemplate(){
        ...
        //准备快递公司数据
        $express = Db::name('express')->where(['dataFlag'=>1,'isShow'=>1])->column('expressName');
		require Env::get('root_path') . 'extend/phpexcel/PHPExcel.php';
		$name = '待发货商品模板';
		$objPHPExcel = new \PHPExcel();
		// 设置excel文档的属性
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', '订单编号')->setCellValue('B1', '订单商品编号')->setCellValue('C1', '商品名称')->setCellValue('D1', '快递公司')->setCellValue('E1', '快递单号');
    ...
    public function editOrderReceiveInfo(){
        ...
        if($address['userName']==''||$address['userAddress']==''||$address['userPhone']=='')return WSTReturn('请填写完整的收货信息');
    ...
    public function getOrderExpressInfo(){
        ...
        $rs = $this->alias('o')
            ...
            ->join('__SHOP_EXPRESS__ se','se.id=oe.expressId', 'inner')
            ->join('__EXPRESS__ e','se.expressId=e.expressId','left')
            ->where(['o.orderId'=>$orderId,'oe.isExpress'=>1])->field('o.orderId,oe.id,e.expressName,oe.expressId,oe.expressNo')->select();
        return $rs;
    }
    ...
    public function editOrderExpressInfo(){
        ...
        if(empty($order))return WSTReturn('无效的订单');
        if(count($expressIdArr)>0){
            for($i=0;$i<count($expressIdArr);$i++){
                if($expressIdArr[$i]=='' || $expressNoArr[$i]=='')return WSTReturn('请填写物流信息');
            }
        }
        Db::startTrans();
        try{
	 ...
	 * 商家(门店)拒收订单
	 */
	public function shopReject($sId=0, $storeId=0){
		$shopId = ($sId==0)?(int)session('WST_USER.shopId'):$sId;
		$orderId = (int)input('post.id');
		// hook('beforeRejectOrder',['uId'=>$uId,'orderId'=>$orderId]);
		$reason = (int)input('post.reason');
		...
		// 查询自提订单
		$order = $this->alias('o')->join('__SHOPS__ s','o.shopId=s.shopId','left')
		              ->where(['o.shopId'=>$shopId,'o.orderId'=>$orderId,
					  		   'o.orderStatus'=>0,'o.deliverType'=>1
					  ])
		              ->field('o.orderId,o.orderNo,o.shopId,s.userId,payType,o.userAddress,o.userName,o.realTotalMoney,o.scoreMoney')->find();
		$reasonData = WSTDatas('SHOP_REJECT_RECEIVE',$reason);
		if(empty($reasonData))return WSTReturn("无效的拒收原因");
		if($reason==10000 && $content=='')return WSTReturn("请输入拒收原因");
		if(!empty($order)){
			Db::startTrans();
		    try{
				$data = ['orderStatus'=>-3,'shopRejectReason'=>$reason];
				if($reason==10000)$data['shopRejectOtherReason'] = $content;
				//如果是货到付款拒收的话，把实付金额设置为0
				if($order['payType']==0)$data['realTotalMoney'] = 0;
			    $result = $this->where('orderId',$order['orderId'])->update($data);
				if(false != $result){
					...
					if($storeId>0){
						// 查找门店userId
						$logUserId = Db::name("store_users")->where(["storeId"=>$storeId, 'shopId'=>$shopId])->value("userId");
					}
					//新增订单日志
					$logOrder = [];
					$logOrder['orderId'] = $orderId;
					$logOrder['orderStatus'] = -3;
					$logOrder['logContent'] = ($storeId>0?'门店':'商家')."拒收订单，拒收原因：".$reasonData['dataName'].(($reason==10000)?"-".$content:"");
					$logOrder['logUserId'] = $logUserId;
					$logOrder['logType'] = 0;
					$logOrder['logTime'] = date('Y-m-d H:i:s');
					Db::name('log_orders')->insert($logOrder);

					// //发送一条用户信息
					// $tpl = WSTMsgTemplates('SHOP_REJECT_RECEIVE');
	                // if( $tpl['tplContent']!='' && $tpl['status']=='1'){
	                //     $find = ['${ORDER_NO}','${REASON}'];
	                //     $replace = [$order['orderNo'],$reasonData['dataName'].(($reason==10000)?"-".$content:"")];
    ...
    public function getShopRemarks(){
        $orderId = (int)input('orderId');
        $orders = Db::name('orders')->where([['orderId','=',$orderId],['dataFlag','=',1]])->field('orderId,orderNo,orderShopRemarks')->find();
        if(empty($orders))return WSTReturn('无效的订单');
        return WSTReturn('',1,$orders);
    }
    /**
     * 修改订单备注
     */
    public function editShopRemarks(){
        $orderId = (int)input('orderId');
        $orderShopRemarks = input('orderShopRemarks');
        $chk = Db::name('orders')->where([['orderId','=',$orderId],['dataFlag','=',1]])->count();
        if($chk==0)return WSTReturn('订单不存在');
        Db::name('orders')->where([['orderId','=',$orderId],['dataFlag','=',1]])->update(['orderShopRemarks'=>$orderShopRemarks]);
        return WSTReturn('操作成功',1);
    }
}
