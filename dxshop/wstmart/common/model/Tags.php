...
class Tags extends Base{
	...
	*/
	public function getGuessLike($catId,$num,$goodsIds=[]){
		...
		if($module=='app'||$module=='weapp'|| !empty($goodsIds)){
			$ids = $goodsIds;
		}else{
	...
	public function listByGoods($type,$catId,$num,$cache = 0){
        ...
        WSTCache('TAG_GOODS_'.$type."_".$catId."_".$num."_".$cache,$goods,$cache);
        return $goods;
	}
	...
	public function listAds($positionCode,$num,$cache = 0,$sortType=0){
		...
		WSTCache('TAG_ADS'.$positionCode."_".$cache,$rs,$cache);
		return $rs;
	}
	...
	public function listFriendlink($num,$cache = 0){
		...
		WSTCache('TAG_FRIENDLINK_'.$cache,$rs,$cache);
	    return $rs;
	}
	...
	public function listArticle($catId,$num,$cache = 0){
		...
		WSTCache('TAG_ARTICLES_'.$catId."_".$num."_".$cache,$rs,$cache);
		return $rs;
	}
    /**
	 * 获取最新文章
	...
	public function listByNewArticle($num,$cache){
		...
		WSTCache('TAG_NEW_ARTICLES_'.$cache,$rs,$cache);
	    return $rs;
	}
	/**
	 * 获取指定分类的文章
	...
	*/
	public function listBrand($catId,$num,$cache = 0){
        ...
        WSTCache('TAG_BRANDS_'.$catId."_".$cache,$brands,$cache);
        return $brands;
	}
	// 二位数组去重
	...
	*/
	public function listShop($catId,$num,$cache = 0){
        ...
        WSTCache('TAG_SHOPS_'.$catId."_".$cache,$shops,$cache);
        return $shops;
	}
	...
	public function listShopGoods($type,$shopId,$cat = 0,$num = 0,$cache = 0){
        ...
        $goods = Db::name('goods')
                   ...
                   ->where($where)->field('g.goodsId,goodsName,goodsImg,goodsSn,goodsStock,saleNum,shopPrice,marketPrice,isSpec,appraiseNum,visitNum,gs.totalScore,gs.totalUsers')
                   ->order($order[$type])->limit($num)->select();
        $ids = [];
        foreach($goods as $key =>$v){
        	if($v['isSpec']==1)$ids[] = $v['goodsId'];
