...
class Users extends Base{
     ...
     * @param userId 用户id
     */
    public function qrCodeLogin($userId){
        $rs = $this->where("userId='{$userId}'")
                    ->where('dataFlag=1 and userStatus=1')
                    ->find();
        hook("beforeUserLogin",["user"=>&$rs,'loginType'=>'qrScanner']);
        return model('common/users')->doLogin($rs, '', 0, 2, 0);
    }
    ...
    public function checkLogin($loginSrc = 0){
    	...
    	$rs = $this->where("loginName|userEmail|userPhone",$loginName)
    				...
    				->find();
    	if(empty($rs)){
            return WSTReturn('该帐号不存在');
        }
        if($rs['userStatus']==-1){
            return WSTReturn('该帐号已申请注销，不能登录');
        }
        if($rs['userStatus']==0){
            return WSTReturn('该帐号已冻结，不能登录');
        }
        hook("beforeUserLogin",["user"=>&$rs,'loginType'=>'account']);
        if(!empty($rs)){
          //  if($rs['loginPwd']!=md5($loginPwd.$rs['loginSecret'])) return WSTReturn("密码错误");
        }
    	return $this->doLogin($rs, $loginName, $rememberPwd, $typ, $loginSrc);

    }
    /**
     * 执行登录
     * $rs 用户在数据库中的信息
     * loginName 登录名，用户记住用户名
     * rememberPwd 是否记住密码
     * typ 登录类型 2:商家
     * loginSrc 登录来源
     */
    public function doLogin($rs, $loginName, $rememberPwd, $typ, $loginSrc){
        if(!empty($rs)){
    ...
    public function regist($loginSrc = 0){
    	...
    	if($wxOpenId){
            ...
            $cuser = Db::name("users")->where(['dataFlag'=>1,'wxOpenId'=>$wxOpenId])->find();
            if(!empty($cuser) && $cuser['userStatus']==-1){
                return WSTReturn('您已申请帐号注销，不能注册');
            }
			...
			if($userinfo){
                ...
                $data['wxUnionId'] = $wxUnionId;
                if($wxUnionId){
                    $cuser = Db::name("users")->where(['dataFlag'=>1,'wxUnionId'=>$wxUnionId])->find();
                    if(!empty($cuser) && $cuser['userStatus']==-1){
                        return WSTReturn('您已申请帐号注销，不能注册');
                    }
                }
    ...
    * 绑定邮箱
    ...
    public function resetPass($uId=0){
    	...
    	if(false !== $rc){
            ...
            hook("afterEditPass",["userId"=>$reset_userId]);
            WSTDelAppToken($reset_userId);
    ...
    public function wxLogout($loginSrc=0,$uId=0){
        // 如果开启了自动注册就不往下执行
        if($loginSrc==1 && (int)WSTConf('CONF.wechatAutoRegister')==1)return WSTReturn("非法退出");
        if($loginSrc==5 && (int)WSTConf('CONF.weappAutoRegister')==1)return WSTReturn("非法退出");
     ...
     * 提交注销申请
     */
    public function applyCancelAcc($userId=0){
        $userId = ($userId>0)?$userId:(int)session('WST_USER.userId');
        $user = Db::name("users")->where(['userId'=>$userId,'dataFlag'=>1])->field('userStatus,userMoney')->find();
        if($user['userStatus']!=1){
            return WSTReturn("您的帐号已处于冻结状态，不能注销");
        }
        if($user['userMoney']>0){
            return WSTReturn("您的帐户钱包余额大于0，不能注销");
        }
        //检查是否有未批准的提现申请
        $cnt = Db::name("cash_draws")->where(['targetType'=>0,'targetId'=>$userId,'cashSatus'=>0])->count();
        if($cnt>0){
            return WSTReturn("您的帐户有未完成的提现申请，不能注销");
        }
        //判断是否有无完成的订单
        $where = [];
        $where[] = ['userId','=',$userId];
        $where[] = ['orderStatus','in',[0,1]];
        $cnt = Db::name("orders")->where($where)->count();
        if($cnt>0){
            return WSTReturn("您的帐户有未完成的订单，不能注销");
        }
        //判断是否存在已取消未退款的订单
        $where = [];
        $where[] = ['userId','=',$userId];
        ...
        $cnt = Db::name("orders")->where($where)->count();
        if($cnt>0){
            return WSTReturn("您的帐户有未退款订单，不能注销");
        }
        ...
        $where[] = ['os.isClose','=',0];
        $cnt = Db::name("orders")->alias('o')
                ->join("order_services os","o.orderId=os.orderId")
                ->where($where)->count();
        if($cnt>0){
            return WSTReturn("您的帐户有销售未完成订单，不能注销");
        }
        //判断帐号是否有开通商家
        $cnt = Db::name("shops")->where(['userId'=>$userId,'dataFlag'=>1])->count();
        if($cnt>0){
            return WSTReturn("您的帐户有开通商家店铺，不能注销");
        }
        //判断帐号是否有开通商家
        $cnt = Db::name("suppliers")->where(['userId'=>$userId,'dataFlag'=>1])->count();
        if($cnt>0){
            return WSTReturn("您的帐户有开通供货商，不能注销");
        }
        Db::startTrans();
        try{
            //修改用户状态为注销
            Db::name('users')->where(['userId'=>$userId,'dataFlag'=>1,'userStatus'=>1,])->update(['userStatus'=>-1,'cancelRemarks'=>'用户申请注销帐号','applyCancelTime'=>date("Y-m-d H:i:s")]);
            //退款系统
            $this->logout();
            WSTDelAppToken($userId);
            Db::commit();
            return WSTReturn("您的注销申请已提交",1);
        }catch (\Exception $e) {
            Db::rollback();
            return WSTReturn('注销申请失败',-1);
        }
