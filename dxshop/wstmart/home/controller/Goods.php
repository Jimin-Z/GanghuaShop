...
class Goods extends Base{
    ...
    public function search(){
    	...
    	$data['orderBy'] = input('orderBy/d',5);
    ...
    public function detail(){
    	...
    	if(!empty($goods)){
            ...
            // 分类信息
            $catInfo = Db::name("goods_cats")->field("detailTheme")->where(['catId'=>$goods['goodsCatId']])->find();
            if(empty($catInfo))
                  $catInfo = Db::name("goods_cats")->field("detailTheme")->find();
            if(!isset($catInfo['detailTheme'])) $catInfo=array('detailTheme'=>'');            // 商品详情延迟加载
            $rule = '/<img src="\/(upload.*?)"/';
	...
	public function contrastGoods(){
		...
		if($id>0){
            ...
            if(empty($goods))return WSTReturn('商品不存在或未上架');
