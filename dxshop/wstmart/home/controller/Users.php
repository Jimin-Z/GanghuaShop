...
class Users extends Base{
	...
	public function login(){
        ...
        if(!empty($prevToken)){
            ...
            cookie('WSTMART_QR_TOKEN_STATUS'.$prevToken, null);
        }
        if(in_array(3, explode(',',WSTConf('CONF.loginType')))){
            $token = WSTToGuidString(3);
            // 缓存token,有效期为2分钟
            cookie('WSTMART_CURRENT_QR_TOKEN', $token, 2 * 60);
            cache('WSTMART_QR_TOKEN'.$token, $token, 2 * 60);
            cookie('WSTMART_QR_TOKEN_STATUS'.$token, 'waiting', 0);
        }

		return $this->fetch('user_login', ['token'=>$token]);
	}
	...
	public function getPhoneVerifyCode(){
        ...
        if( $tpl['tplContent']!='' && $tpl['status']=='1'){
            ...
            // $rv = ['status'=>1,'msg'=>'短信发送'];
            // $rv =['msg'=>'短信发送成功!','status'=>1,'checkcode'=>$phoneVerify];
        }
		if($rv['status']==1){
			session('VerifyCode_userPhone',$userPhone);
			session('VerifyCode_userPhone_Verify',$rv['checkcode']);
    ...
    */
    ...
    public function toCancel(){
        ...
        $data = $m->getById($userId);
        $this->assign('data',$data);
        return $this->fetch('users/security/account_cancel');
    }
    ...
    public function cancelProtocol(){
        $rs = model('common/articles')->getById(301);
        $this->assign('data',$rs);
        return $this->fetch("users/security/cancel_protocol");
    }

    public function applyCancelAcc(){
        $m = new MUsers();
        $rs = $m->applyCancelAcc();
        return $rs;
    }
