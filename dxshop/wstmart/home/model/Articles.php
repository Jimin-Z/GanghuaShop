...
class Articles extends Base{
		...
		if(!$arts){
			$rs = $this->alias('a')->join('article_cats ac','a.catId=ac.catId','inner')
				  ...
				  ->where(['a.dataFlag'=>1,
				  		   ...
				  		   'ac.isShow'=>1,
				  		   'ac.parentId'=>7])
				  ->order('a.catSort asc,a.articleId asc')
				  ->select();
			//同一分类下的文章放一起
			$catName = [];
			$arts = [];
	...
	*/
	public function getHelpById(){
		...
		$rs = $this->alias('a')->join('__ARTICLE_CATS__ ac','a.catId=ac.catId','inner')->where('ac.parentId=7 and  a.dataFlag=1 and a.isShow=1 and a.articleId='.$id)->find();
	...
	*/
	...
	public function getTree(){
		...
		if(!$artTree){
			$data = Db::name('article_cats')->field('catName,catId,parentId')->where('parentId <> 7 and catId <> 7 and dataFlag=1 and isShow=1')->select();
