...
class Goods extends CGoods{
	 ...
	 */
     public function getBySale($goodsId){
		...
		if(!empty($rs)){
			...
			if(!isset($rs['scores']['totalScore'])){
				$rs['scores']['totalScore']=0;$rs['scores']['totalUsers']=0;
			}
	...
	public function pageQuery($goodsCatIds = []){
        ...
        // 是否第一次进来
        $isDefault = input('orderBy','');
        $isDefault = ($isDefault=='' || $isDefault==5)?1:0;
		//排序条件
		$orderBy = input('orderBy/d',5);
		...
		if($orderBy==5){//综合排序
        	$list = $list->orderRaw(WSTGoodsListDefaultOrder())->paginate(input('pagesize/d',20))->toArray();
        }else{
