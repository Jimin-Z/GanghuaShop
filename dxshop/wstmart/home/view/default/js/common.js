...
WST.shopWangWang = function(val){
	if(WST.blank(val) !=''){
		return [
	           '<a target="_blank" href='+window.conf.HTTP+'qmdd.net/welive/kefu.php?a=6168&g=1&ver=3&touid='+val+'&siteid=cntaobao&status=1&charset=utf-8">',
		       '<span class="order_from">联系商家</span>',
	           '</a>'
		       ].join('');
	}else{
		return '';
