...
function showProtocol(){
    layer.open({
        ...
        area: ['1000px', '90%'],
        offset:'20px',
        content: [WST.U('home/users/cancelprotocol')],
        btn: ['同意并注销'],
...
function applyCancelAcc(){
    ...
    if(!protocol){
        WST.msg('请选择我已阅读并同意协议');
        return;
    }
    WST.confirm({content:"您确定要申请注销该帐号吗？",yes:function(tips){
        var loading = WST.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
        $.post(WST.U('home/users/applyCancelAcc'),{},function(data,textStatus){
            layer.close(loading);
            var json = WST.toJson(data);
            if(json.status=='1'){
