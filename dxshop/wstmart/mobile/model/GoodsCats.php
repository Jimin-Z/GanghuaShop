...
class GoodsCats extends Base{
	...
	public function getGoodsCats(){
		$list = WSTCache('WST_CACHE_GOODS_CAT_MOB');
		if(!$list){
			//查询一级分类
			...
			$ids[] = -1;
			//查询二级分类
			$trs2s = $this->where("dataFlag=1 and isShow=1 and parentId in(".implode(',',$ids).")")->field('catId,parentId,simpleName')->order('catSort asc')->select();
			$trs2 = array();
			$ids = array();
			...
			$ids[] = -1;
			//查询三级分类
			$trs3s = $this->where("dataFlag=1 and isShow=1 and parentId in(".implode(',',$ids).")")->field('catId,parentId,simpleName,catImg')->order('catSort asc')->select();
			$trs3 = array();
			$ids = array();
			...
			WSTCache('WST_CACHE_GOODS_CAT_MOB',$list,86400);
		}
		return $list;
	}
