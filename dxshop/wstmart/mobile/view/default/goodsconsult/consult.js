...
function dataShow(){
    ...
    jQuery('#frame').show().animate({"bottom": 0}, 500);
}
...
function consultCommit(){
  ...
  WST.load('正在提交，请稍后...');
  $('.wst-ad-footer .button').addClass("active").attr('disabled', 'disabled');
  $.post(WST.U('mobile/goodsconsult/add'),params,function(responData){
    ...
    if(json.status==1){
       ...
       setTimeout(function(){
        ...
        setTimeout(function(){
            $('.wst-ad-footer .button').removeAttr('disabled').removeClass("active");
        },1500);
    }
  })
}
