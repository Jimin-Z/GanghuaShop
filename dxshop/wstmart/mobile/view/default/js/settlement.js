...
$(function(){
	$('#special_invoice_head').focus(function(){
		...
		var specialInvoiceNum = 0;
		$('#special_inv_headlist .inv_list_item li').each(function(idx,item){
			if($(item).hasClass('j-special')){
				specialInvoiceNum+=1;
			}
		});
		if(specialInvoiceNum==0){
			$('#special_inv_headlist .inv_list_item').html("<li onclick='javascript:WST.goInvoice()'>暂无专用发票，点击此处新增</li>");
		}
	})
	...
	$('#invoice_head').focus(function(){
		$('#inv_headlist').show();
		var normalInvoiceNum = 0;
		$('#inv_headlist .inv_list_item li').each(function(idx,item){
			if($(item).hasClass('j-normal')){
				normalInvoiceNum+=1;
			}
		});
		if(normalInvoiceNum==0){
			$('#inv_headlist .inv_list_item').html("<li onclick='javascript:WST.goInvoice()'>暂无普通发票，点击此处新增</li>");
		}
	})
...
function setInvoiceText(){
	...
	var invoiceObj = $('#invoice_obj').val();// 发票对象
	var invoiceHead = $('#invoice_head').val();// 发票抬头
	var invoiceType = $('#invoiceType_'+currShopId).val();// 发票类型
	var text = '不开发票';
	var _taxText = invoiceType==1?'专用':'普通';
	if(isInvoice==1){
		text = (invoiceObj==0)?(_taxText+'发票  个人   明细'):(_taxText+'发票<br />'+invoiceHead+'<br />明细');
	}
	if((invoiceObj==0)){
...
function inDetermine(n){
	...
	if(n=='gives'){
		var _uPhone = $("#j-userPhone-"+currStoreShopId);
...
function invTypeSwitch(obj, n){
	...
	var mode = 0;
	$('.inv_unit').each(function(idx,item){
		if($(item).hasClass('wst-active')){
			mode = $(item).attr('mode');
		}
	});
	if(n==0 && mode==1){
		$('#invoice_obj').val(mode);
	}else{
		$('#invoice_obj').val(n);
	}
        ...
        var gettpl1 = document.getElementById('invoiceBox').innerHTML;
          laytpl(gettpl1).render(json, function(html){
				...
				$('.inv_list_item li').click(function(){
					...
					if(invoiceType==1){
						$('#j-special-detail-box').show();
						$('#specialInvoiceTitle').html($(this).html());
						$('#specialInvoiceCode').html($(this).attr('invCode'));
						$('#invoiceAddr').html($(this).attr('invoiceAddr'));
						$('#invoicePhoneNumber').html($(this).attr('invoicePhoneNumber'));
						$('#invoiceBankName').html($(this).attr('invoiceBankName'));
						$('#invoiceBankNo').html($(this).attr('invoiceBankNo'));
					}else{
						$('#j-normal-detail-box').show();
						$('#normalInvoiceTitle').html($(this).html());
						$('#normalInvoiceCode').html($(this).attr('invCode'));
					}
				})
          });
      }else{
