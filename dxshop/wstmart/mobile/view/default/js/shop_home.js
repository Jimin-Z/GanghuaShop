    ...
    if(pageId==undefined) {
        ...
        $(window).scroll(function () {
            ...
            if ((($(window).scrollTop() + $(window).height()) + 50) >= $(document).height()) {
                if (parseInt($('#currPage').val(),10) < parseInt($('#totalPage').val(),10))shopsList();
            }
        });
    }else{
        new Swiper('.banner', {
    ...
    $.post(WST.U('mobile/shops/getShopGoods'), param, function(data){
        ...
        if(json && json.data && json.data.length>0){
            ...
            if(json.current_page>=json.last_page){
                $('.wst-load-text').html("<div class='wst-load-line'></div><div class='wst-load-text-content'>我是有底线的</div><div class='wst-load-line'></div>");
            }else{
