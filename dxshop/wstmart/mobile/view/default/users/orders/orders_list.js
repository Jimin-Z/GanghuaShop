...
function cancelOrder(oid){
    ...
    $('#wst-event0').attr('disabled', 'disabled');
  $.post(WST.U('mobile/orders/cancellation'),{id:oid,reason:$('#reason').val()},function(data){
    ...
    if(json.status==1){
      ...
      WST.msg(json.msg,'info');
      $('#wst-event0').removeAttr('disabled');
    }
  });
}
...
function rejectOrder(oid){
  ...
  $('#wst-event3').attr('disabled', 'disabled');
  $.post(WST.U('mobile/orders/reject'),param,function(data){
    ...
    if(json.status==1){
      ...
      WST.msg(json.msg,'info');
      $('#wst-event3').removeAttr('disabled');
    }
  });
...
function refund(id){
  var params = {};
      ...
      $('#wst-event8').addClass("active").attr('disabled', 'disabled');
      $.post(WST.U('mobile/orderrefunds/refund'),params,function(data){
          ...
          if(json.status==1){
            ...
            WST.msg(json.msg,'info');
            $('#wst-event8').removeAttr('disabled').removeClass("active");
          }
      })
}
