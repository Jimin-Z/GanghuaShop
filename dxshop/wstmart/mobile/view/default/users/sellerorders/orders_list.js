...
function rejectOrder(oid) {
  ...
  $.post(WST.U('mobile/orders/shopReject'), param, function (data) {
...
function showEditShopRemarkBox(id,event){
    $("#wst-event5").attr("onclick","javascript:"+event);
    $("#editShopRemarkBox").dialog("show");
    $.post(WST.U('mobile/orders/getShopRemarks'),{orderId:id},function(data){
        var json = WST.toJson(data);
        if(json.status>0){
            $('#orderShopRemarks').val(json.data.orderShopRemarks);
        }
    });
}

function editShopRemark(oid){
    var orderShopRemarks = $('#orderShopRemarks').val();
    $.post(WST.U('mobile/orders/editShopRemarks'),{orderId:oid,orderShopRemarks:orderShopRemarks},function(data){
        hideDialog('#editShopRemarkBox');
        var json = WST.toJson(data);
        if(json.status>0){
            WST.msg(json.msg,'success');
            setTimeout(function(){
