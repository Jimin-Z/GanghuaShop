...
class GoodsAppraises extends Base
{
    public function queryByPage($sId = 0)
    {
       ...
       // $where[] = ['ga.dataFlag', "=", 1];
        ...
        $model = model('goods');
        $data = $model->alias('g')
            ->field('g.goodsId,g.goodsImg,g.goodsName,ga.shopReply,ga.id gaId,ga.replyTime,ga.goodsScore,ga.serviceScore,ga.timeScore,ga.content,ga.images,u.loginName,o.orderNo,ga.createTime,ga.isReport')
            ->join('__GOODS_APPRAISES__ ga', 'g.goodsId=ga.goodsId', 'inner')
             ->join('__ORDERS__ o','o.orderId=ga.orderId')
