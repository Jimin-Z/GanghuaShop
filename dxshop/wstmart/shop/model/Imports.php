    ...
    public function importGoods($data){
        ...
        try{
            ...
            for ($row = 3; $row <= $rows; $row++){//行数是以第3行开始
                ...
                $goods['shopCatId1'] = 0;
                $goods['shopCatId2'] = 0;
                $goods['goodsUnit'] = trim($sheet->getCell("I".$row)->getValue());
                ...
                if($goods['isFreeShipping']==1){
                    ...
                    if($shopExpressText!=""){
                        ...
                        $where[] = ["se.dataFlag","=",1];
                        $where[] = ["e.dataFlag","=",1];
                        $where[] = ["e.expressName","=",$shopExpressText];
                        ...
                        if(!empty($shopExpress)){
                            ...
                            $specGoodsErrMsgArr[] = ['msg'=>"商品【".$goods['goodsName']."】快递公司【".$shopExpressText."】未启用导入失败，已跳过"];
                        }
                    }
                }
                ...
                if(!empty($goodsCat)){
                    ...
                    if(isset($goodsCatMap[$goodsCat])){
                        ...
                        if(count($goodsCatId)>0){
                            ...
                            if(count($goodsCatId)==1 && isset($goodsCatId[0]))$goodsCatId = $goodsCatId[0];
                        }
                        ...
                        if(!empty($goodsCatId['catId']) && in_array($goodsCatId['catId'],$applyCatIds)){
                            ...
                            $specGoodsErrMsgArr[] = ['msg'=>"商品【".$goods['goodsName']."】导入失败，商城分类未申请"];
                            continue;
                        }
                    }
                }
                ...
                if($sheetName!=''){// 存在规格页的key
                    ...
                    if($currSheet!=null){// 规格页存在
                        ...
                        foreach($specKeyArr as $catName=>$v){
                            $catNameData = Db::name('spec_cats')->where(['catName'=>$catName,'dataFlag'=>1])
                        ...
                        if($findCount===$existsCount){
                            ...
                            if($specItemTotal===($_rows-1)){
                                ...
                                if(count($gspecArray)>0){
                                    ...
                                    //更新默认价格和总库存
                                    Db::name('goods')->where('goodsId',$goodsId)->update(['isSpec'=>1,'shopPrice'=>$defaultPrice,'costPrice'=>$costPrice,'goodsStock'=>$totalStock]);
                                }
                            }else{
                                /**
                                 * 错误信息：组合总数不匹配
 ...
 */
    ...
    */
    ...
    public function importOrders($data){
        ...
        ->alias("e")
        ->join("__SHOP_EXPRESS__ se",'se.expressId=e.expressId', 'inner')
        ->where(['se.dataFlag'=>1,'se.isEnable'=>1,'se.shopId'=>$shopId])->column('se.id expressId','e.expressName');
