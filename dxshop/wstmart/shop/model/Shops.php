...
class Shops extends CShops{
    ...
    */
    ...
    public function getShopSummary($shopId){
        ...
        //售后信息
        $stat['orderServicesCnt'] = Db::name('orders')->alias('o')->join('order_services orf','orf.orderId=o.orderId')->where([['o.shopId','=',$shopId], ['orf.isClose', "=", 0], ['orf.serviceStatus','in',[0,2,3]],['o.dataFlag','=',1]])->count();
        //库存预警
        ...
        //退货退款订单
        $stat['orderRefundCnt'] = Db::name('orders')->alias('o')->join('order_refunds orf','orf.orderId=o.orderId')->where(['shopId'=>$shopId,'orf.refundStatus'=>0,'o.dataFlag'=>1])->count();
        //待评价订单
        ...
        $stat['cartCnt'] = Db::name("carts")->alias('c')
                                ->join("goods g","c.goodsId=g.goodsId")
                                ->where(['g.shopId'=>$shopId,'g.dataFlag'=>1])
                                ->count();
        $stat['collectionCnt'] = Db::name("favorites")->alias('f')
    ...
    */
    ...
    public function closeGuide(){
        $shopId = (int)session('WST_USER.shopId');
        $this->where('shopId',$shopId)->update(['isGuide'=>1]);
    }
