function stockByPage(id,p){
    ...
    var cols = [
		{type:'checkbox'},
        {title:'卡劵号', field:'cardNo'},
        {title:'密码', field:'cardPwd'},
        {title:'状态', field:'isUse', templet: function(item,rowIndex){
                var isUse="";
        ...
        {title:'消费订单', field:'orderNo'},
        {title:'操作', field:'' ,width:300, align:'center', templet: function(item,rowIndex){
                var h = "";
                if(item['isUse']==0)h += "<a class='btn btn-blue' href='javascript:editCard("+item['id']+")'><i class='fa fa-pencil'></i>编辑</a> ";
	...
	mmg.render({
		elem: '.mmg',
		id:'dataTable',
		url:WST.U('shop/goodsvirtuals/stockByPage',"id="+id),
		cellMinWidth: 80,
