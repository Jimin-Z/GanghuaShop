...
WST.createShopQrcode = function(vtype){
	...
	if(vtype==1){
		$('#'+qrbox).html(loading+"正在生成手机/微信二维码");
	}else{
		$('#'+qrbox).html(loading+"小程序二维码");
	}
    $.post(WST.U('shop/shops/createShopQrcode'),params,function(data,textStatus){
    	...
    	if(json.status=='1'){
    		...
    		html.push("<a href='"+WST.conf.RESOURCE_PATH+"/"+json.data+"' download style='font-weight:bold;margin-left:20px;'>下载二维码</a>");
    		$('#'+qrbox).html(html);
    	}else{
    		$('#'+qrbox).html(json.msg).css({'color':'red','padding-top':'30px'});
    	}
    });
}
 ...
 * 变换列表按钮/按钮组
 */
WST.initGridButtons = function(opts){
	if(opts.btns.length>0 && opts.btns[0].type=='-')opts.btns.splice(0,1);
	if(opts.btns.length>0 && opts.btns[opts.btns.length-1].type=='-')opts.btns.splice(opts.btns.length-1,1);
	if(opts.btns.length==0)$(opts.id).remove();
	if(opts.btns.length>1){
		var _opts = {
			elem: opts.id,
			trigger: 'hover',
			data: opts.btns,
		...
		layui.dropdown.render(_opts);
	}else{
		var _domObj = $(opts.id);
		var _btnObj = opts.btns[0];
		if(!_btnObj || _btnObj.type=="-"){
			_domObj.remove();
			return;
		}
		_domObj.attr({"class":"btn "+_btnObj.cls});
		_domObj.click(function(){
			var _fn = window[_btnObj.clickFun];
			if(_domObj.data('fun-'+_btnObj.clickFun)){
				_fn && eval(_btnObj.clickFun+'('+_domObj.data('fun-'+_btnObj.clickFun)+')');
			}else{
				_fn && _fn($(this).data("id"));
			}

		})
		_domObj.html('<i class="fa '+_btnObj.icon+'"></i>'+_btnObj.title);
 ...
 * 获取表格高度
 */
WST.initGridHeight = function(oheight){
	var tbHeight = $('.layui-tab-title').outerHeight(true)+$('#alertTips').outerHeight(true)+$('.wst-toolbar').outerHeight(true)+30;
	if(typeof(oheight)!='undefined')tbHeight = tbHeight + oheight;
	return 'full-'+tbHeight;
}

WST.initTabHeight = function(oheight){
	var tbHeight = $('.layui-tab-title').outerHeight(true)+$('#alertTips').outerHeight(true)+$('.wst-toolbar').outerHeight(true)+30;
	var h = WST.pageHeight();
	$("#dataTable").height(h-tbHeight-oheight);
}

WST.showPhotos = function(id,isParent){
	var objs = $(id).children();
	var json = {"start": 0,"data":[]};
	for(var i=0;i<objs.length;i++){
		$(objs[i]).attr('n',i);
		json["data"].push({"src":$(objs[i]).attr('layer-src'),"thumb":$(objs[i]).attr('src')});
	}
	for(var i=0;i<objs.length;i++){
		$(objs[i]).click(function(){
			json['start'] = $(this).attr('n');
			if(isParent){
		        parent.showImg({photos: json,anim: 5 });
		    }else{
