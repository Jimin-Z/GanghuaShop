...
function setPaypwd(){
	layerbox =	layer.open({
		title:['设置店铺支付密码','text-align:left'],
		type: 1,
		area: ['450px', '240px'],
		content: $('.j-paypwd-box'),
		btn: ['设置店铺支付密码，并支付年费', '关闭'],
		yes: function(index, layero){
			...
			$.post(WST.U('shop/shops/payPassEdit'),{newPass:newPass,reNewPass:reNewPass},function(data,textStatus){
				layer.close(load);
				var json = WST.toJson(data);
				if(json.status==1){
					WST.msg(json.msg, {icon: 1,time:1500},function(){
