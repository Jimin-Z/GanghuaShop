...
function loadStat(){
	...
	$.post(WST.U('shop/reports/getStatFavorites'),WST.getParams('.j-ipt'),function(data,textStatus){
		layer.close(loading);
...
function getStatFavoriteGoods(p){
	mmg = layui.table;
	mmg.render({elem: '#mmg',id:'dataTable',url:WST.U('shop/reports/getStatFavoriteGoods'),cellMinWidth: 80,height: WST.initGridHeight(),skin: 'line',even: true,limit:20,page:true,
		cols: [[
			{title:'#',type:'numbers'},
			{type: 'checkbox', field:'cartId'},
			{title:'收藏日期', field:'createTime',width: 200},
