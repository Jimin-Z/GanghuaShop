...
$(function(){
    ...
    $('#shopPhoneVerify').validator({
        valid: function(form){
            var n=$('#VerifyId').val();
            getShopPhoneVerifys(n);
        }
    });
})
function vePayForm(){
...
// 店铺支付密码
//发送手机验证码
function getShopPhoneVerify(n){
    if(!$('#userPhone').isValid())return;
    $('#VerifyId').val(n);
    if(window.conf.SMS_VERFY==1){
        WST.open({type: 1,title:"请输入验证码",shade: [0.6, '#000'],border: [0],content: $('#shopPhoneVerify'),area: ['600px', '240px'],end: function () {
                $('#shopPhoneVerify').hide();
            }});
    }else{
        getShopPhoneVerifys(n);
    }
}
function getShopPhoneVerifys(n){
    WST.msg('正在发送短信，请稍后...',{time:600000});
    var time = 0;
    var isSend = false;
    var params = WST.getParams('.ipt');
    $.post(WST.U('shop/shops/getPhoneVerify'+n),params,function(data,textStatus){
        var json = WST.toJson(data);
        if(isSend )return;
        isSend = true;
        if(json.status!=1){
...
function veShopPayForm(){
    //修改支付密码
    shopPayForm = $('#shoppayform').validator({
        valid: function(form){
            var params = WST.getParams('.ipt');
            if(window.conf.IS_CRYPT=='1'){
                var public_key=$('#token').val();
            ...
            var loading = WST.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
            $.post(WST.U('shop/shops/payPassEdit'),params,function(data,textStatus){
                layer.close(loading);
                var json = WST.toJson(data);
                if(json.status=='1'){
                    WST.msg(json.msg,{icon:1,time:2000},function(){
...
function veGetShopPayForm(){
    //重置支付密码
    getShopPayForm = $('#getShopPayForm').validator({
        valid: function(form){
            var params = WST.getParams('.ipt');
            var loading = WST.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
            $.post(WST.U('shop/shops/payEditt'),params,function(data,textStatus){
                layer.close(loading);
                var json = WST.toJson(data);
                if(json.status=='1'){
                    location.href=WST.U('shop/shops/editPaySut');
                }else{
                    WST.msg(json.msg,{icon:2});
                    WST.getVerify('#verifyImg');
                }
...
function resetShopPayForm(){
    //设置支付密码
    shopResetPayForm = $('#shopResetPayForm').validator({
        valid: function(form){
            var me = this;
            me.holdSubmit();
            var params = WST.getParams('.ipt');
            ...
            var loading = WST.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
            $.post(WST.U('shop/shops/payEdito'),params,function(data,textStatus){
                layer.close(loading);
                var json = WST.toJson(data);
                if(json.status=='1'){
                    location.href=WST.U('shop/shops/editPaySu','pr='+json.process);
                }else{
                    me.holdSubmit(false);
                    WST.msg(json.msg,{icon:2});
                    WST.getVerify('#verifyImg');
