...
function initGrid(p,userId){
	mmg = layui.table;
	mmg.render({elem: '#mmg',id:'dataTable',url:WST.U('shop/shopusers/pageQuery'),cellMinWidth: 80,height: WST.initGridHeight(),skin: 'line',even: true,limit:20,page:{curr:p},
		cols: [[
			{title:'#',type:'numbers'},
			{title:'账号名', field:'loginName',width:200 },
			...
			for(var i=0;i<res.data.length;i++){
				...
				btns.push({title: '接收提醒设置',clickFun:"toNotify", cls:"btn-blue", icon:"fa fa-pencil"});
...
function loadGrid(p){
	p=(p<=1)?1:p;
	mmg.reload('dataTable',{page:{curr:p},where:{userName:$('#userName').val()}});
}
