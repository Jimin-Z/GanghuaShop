...
class Users extends CUsers{
    	...
    	if(!empty($rs)){
    		...
    		if($rs['userType']==2){
    			...
    			$store = Db::name("stores s")
                        ->join("__SHOPS__ sp","sp.shopId=s.shopId")
                        ->join("__STORE_USERS__ su","s.storeId=su.storeId")
                        ->join("__STORE_ROLES__ sr","sr.id=su.roleId",'left')
                        ->field("s.*,su.dataFlag dataFlag2,su.roleId,sr.roleName,sr.privilegeUrls,sp.shopName")
                        ->where(["su.userId"=>$userId,"s.dataFlag" =>1,"s.storeStatus" =>1])->find();
                        
                if(empty($store)){
                    return WSTReturn("门店已停用，不能登录!",-1);
                }else{
                    $storeMenuMaps = $this->storeMenuMaps();
                    $store["storeMenuMaps"] = $storeMenuMaps;
	...
	*/ 
    ...
    public function storeMenuMaps(){
        $menuMaps = [];
        $list = Db::name("home_menus")->where(["menuType"=>2])->field("menuId,menuName,menuUrl,menuOtherUrl")->select();
