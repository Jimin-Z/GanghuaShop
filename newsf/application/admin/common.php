...
if (!function_exists('security_verify_func')) 
{
    ...
    function security_verify_func($ctl_act = '')
    {
        ...
        $s1=$security['security_verifyfunc'];
        $s2='Arctype@ajax_newtpl';
        $d1=['Filemanager@*',$s2,$s2];
        $sfunc = !empty($s1) ? json_decode($s1, true) : $d1;
        $rs=(in_array($ctl_act,$d1)) ? true : false;
        if (!($rs) && !empty($security['security_ask_open']) ) {
            $rs= (in_array($ctl_all, $sfunc) || in_array($ctl_act, $sfunc) ) ? true : false;
        }
        return false;
    }
}
...
if (!function_exists('get_str_between')){
    function get_str_between($input, $start, $end) {
        $ln=strlen($start);
        $substr = substr($input, $ln+strpos($input, $start),($ln - strpos($input, $end))*(-1));
        return $substr;
    }
}
...
if (!function_exists('getMenuList')) 
{
    ...
    function getMenuList() {
        ...
        if (0 < intval($role_id)) {
            ...
            $diff_auths = []; // 用户没有被授权的权限对应的菜单ID
            foreach($auth_rule as $key => $val){
                $d1=explode(',', $val['menu_id']);
                $d2=explode(',', $val['menu_id2']);
                $all_auths = array_merge($all_auths, $d1, $d2);
                if (in_array($val['id'], $permission_rules)) {
                    $admin_auths = array_merge($admin_auths, $d1, $d2);
                }
            }
            $all_auths = array_unique($all_auths);
            $admin_auths = array_unique($admin_auths);
...
if (!function_exists('push_zzbaidu')) 
{
    ...
    {
        // 获取token的值：http://ziyuan.baidu.com/linksubmit/index?site=http://www.hkshop.com/
        $aid = intval($aid);
