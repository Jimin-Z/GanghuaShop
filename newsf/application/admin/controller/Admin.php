...
class Admin extends Base {
     ...
     * 获得类型
     */
    public function gettype($third_login,$tname,$sdata,$sname){
      $login_type=1;
      if ( $third_login==$tname) {
         $login_type= (empty($sdata[$sname])) ?2 :3;
    ...
    public function setLoginType(){
        $type = 1; //仅账号密码登录  2-账号密码登录&微信扫码登录 3-仅微信扫码登录
        $thirdata = login_third_type();
        $tlogin = !empty($thirdata['type']) ? $thirdata['type'] : '';
        $sdata=$thirdata['data'];
        if($type==1) $type=$this->gettype($tlogin,'EyouGzhLogin',$sdata,'force');
        if($type==1) $type=$this->gettype($tlogin,'userGzhLogin',$sdata,'force');
        if($type==1) $type=$this->gettype($tlogin,'WechatLogin',$sdata,'security_wechat_forcelogin');
        $this->assign('login_type', $type);
        $this->assign('third_login', $tlogin);
        return $type;
    }
     ...
     * 检查
     */
    public function checkWebfile(){
       if (session('?admin_id') && session('admin_id') > 0) {
            $web_file = tpCache('global.web_adminbasefile');
            $web_file = !empty($web_file) ? $web_file : $this->root_dir.'/login.php';
            if (stristr($web_file, 'index.php')) {
                $baseFile = explode('/', request()->baseFile());
                $web_file = end($baseFile);
                $web_file = $this->root_dir.'/'.$web_file;
            }
            $this->success("您已登录", $web_file);
        }
    }
    /*
     * 管理员登陆
    ...
    public function login()
    {
        ...
        $this->checkWebfile();//检查是否登录
      
        // 默认开启验证码
    
        $is_vertify=$this->getvertify();
        /*----------------微信扫码登录 start---------------*/
         $login_type=$this->setLoginType();
        /*----------------微信扫码登录 end---------------*/
        $post = input('post.');
        if (IS_POST) {
            ...
            if (!testWriteAble(ROOT_PATH.config('session.path').'/')) {
                $this->error('请仔细检查以下问题：<br/>1、磁盘空间大小是否100%；<br/>2、站点目录权限是否为755；<br/>3、站点所有目录的权限，禁止用root:root ；<br/>4、如还没解决，请点击：<a href="http://www.hkshop.com/wenda/6958.html" target="_blank">查看教程</a>');
            }
                ...
                if (!empty($admin_count)) {
                    ...
                    if ($loginErrnum >= $loginErrtotal) {
                        if (getTime() < $loginErrtime + $loginErrexpire) {
                            ...
                            $surplus_time=($surplus_time <= 0) ? 1 : $surplus_time;
            ...
            if (!empty($user_name) && !empty($password)) {
                ...
                $admin_info = findOne('admin',$condition);
                if (!empty($admin_info)) {
        ...
        $s1=$this->globalConfig['web_theme_login_tplname'];
        $web_tplname = empty($s1) ? '' : $s1;
        if (!empty($web_tplname) && file_exists("application/admin/template/theme/{$web_tplname}")) {
            $login_tplname = str_ireplace('.htm', '', $web_tplname);
            $viewfile = "theme/{$login_tplname}";
        }
        $b=is_dir('./weapp/Mbackend/') && isMobile();
        if ($b) {
            $s1= (2 <= $this->php_servicemeal) ?'zy' :'';
            $viewfile = 'weapp/Mbackend/template/admin/login_'.$s1.'_m.htm';
            // 是否配置微信公众号登录信息
            ...
            if (method_exists($mbackendLogic, 'login_assign')) {
                ...
                $s1=tpSetting("OpenMinicode.conf_wechat");
                $wechat = ($s1) ? json_decode($s1, true) : [];
            ...
            if (function_exists('double_login_logic')) {
                ...
                if (!empty($s1)) $assign_data = array_merge($assign_data, $s1);
            }
        }
        $this->assign($assign_data);
        return $this->fetch(( ($b) ? '' :':')."{$viewfile}");
    
    }

    // 后台管理插件（手机版）--微信公众号登录
    // $assign_data=$this->getPwdJsCode();
    public function getPwdJsCode(){
         // 等保密码复杂度验证 start*/
        $assign_data = [];
    ...
    public function ajax_add_user_name()
    {
        if (IS_AJAX_POST) {
            $user_name = input('post.user_name/s');
            if (findOne('admin',array("user_name"=>$user_name))) {
                $this->error("此用户名已被注册，请更换！");
            }
            $row = findOne('users',['username'  => $user_name,'lang'=> $this->admin_lang]);
            if (!empty($row)) {
...
EOF;
        ...
        $row = findOne('admin_wxlogin',['openid'=>$openid, 'type'=>2]);
        $saveData = [
                'admin_id'   => $admin_id,
...
EOF;
            ...
            $r =updateAll('admin_wxlogin',['wx_id' => $row['wx_id']],$saveData);
        } else {
            $saveData = array_merge( $saveData,['type'  => 2,  'openid'    => $openid,
                'unionid'    => $unionid,'add_time'=>getTime()]);
