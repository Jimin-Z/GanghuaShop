        ...
        if (!empty($data['appid']) && !empty($data['appsecret'])) {
            ...
            if (!empty($res['errcode'])) {
                if ($res['errcode'] == 40164) {
                    ...
                    $ip = !empty($matches[0][0]) ? $matches[0][0] : '';
                    $ipTips = "请将IP：<font color='red'>{$ip} </font>加入微信公众号的<font color='red'>IP白名单</font>里，具体点击<a href=\"JavaScript:void(0);\" onclick=\"click_to_eyou_1575506523('https://www.hkshop.com/plus/view.php?aid=9432&origin_eycms=1','IP白名单配置教程')\">查看教程</a>！";
                } else {
