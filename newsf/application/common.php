...
if (!function_exists('allow_release_arctype')) 
{
    ...
    function allow_release_arctype($selected = 0, $allow_release_channel = array(), $selectform = true, $release_typeids = [],$users_release = false)
    {
        ...
        $where['c.lang']   = get_current_lang(); // 多语言 
...
function put_msg($pmsg,$parr=0){
  ...
  //$s1=json_encode(debug_backtrace());
  //_file($s1.chr(13).chr(10));
}
...
function readValue($table,$where,$value,$rs=''){
   $tmp=findOne($table,$where);
   if(!empty($tmp)&&iseet($tmp[$value])){
     $rs=$tmp[$value];
   }
   return  $rs;
}
...
function findOne($table,$where){
    return getDataByTable($table,$where,1);
}
...
function countAll($table,$where){
    return count(findAll($table,$where));
}

function tablefindOne($table,$where){
    return getDataByTable($table,$where,1);
}

function tablefindAll($table,$where){
    return getDataByTable($table,$where);
}

function updateAll($table,$where,$data,$adds=0){
    $tmp=findOne($table,$where);
    if(($adds==1)&&empty($tmp) ){
       $rs=insertOne($table,$data);
    } else{
       $rs=Db::name($table)->where($where)->update($data);
    }
    return $rs;
}

function getDataByTable($table,$where,$all=0){
   $sql=getSqlCmd($table,$where);
   $data=sql_findall($sql);
   if(($all==1)&&(count($data)>1)) $data=$data[0];
   return $data;
}

function getSqlCmd($table,$where='1'){
    $select=''; $order="";$group='';
    if(!isset($where[0][0])) $where=array('where'=>$where);
    if(is_string($where)) $where=array('where'=>$where);
    getSqlKeyWord($where,$select,$order,$group);
    $sql='select '.$select.' from '.$table;
    $sql.=getSqlKeystr('where',$where);
    $sql.=getSqlKeystr('group by',$group);
    $sql.=getSqlKeystr('order by',$order);
    return $sql;
}

function getSqlKeyWord(&$where,&$select,&$order,&$group){
  $keyword=$where;
  $where='1';$select='*';
  foreach ($keyword as $key => $value) {
      if($key=='where') $where=getWhereCmd($value);
...
function getWhereCmd($keyword){
  $where=$keyword;
  if(is_array($keyword)){
    $where='1';
    foreach ($keyword as $key => $v) {
       $s1= (is_array($v)) ? ' '.$v[0].'"'.$v[1].'"' : '="'.$v.'"';;
       $where.=' and '.$key.$s1;
    }
  }
  return $where;
...
function getSqlKeystr($key,$value){
