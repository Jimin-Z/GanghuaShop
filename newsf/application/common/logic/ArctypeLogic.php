        ...
        if ($res === NULL)
        {
            ...
            if (in_array(MODULE_NAME, ['admin']) && 0 < intval($admin_info['role_id'])) {
                ...
                if(! empty($auth_role_info)){
                    $s1=$auth_role_info['permission']['arctype'];
                    if(! empty($s1)){   $where['id'] = array('IN', $s1);  }
                }
            }
            /*--end*/

            /*多语言 */
            if (empty($map['lang'])) {  $where['lang'] = get_current_lang(); }
            /*--end*/

            if (!empty($map)) {      $where = array_merge($where, $map); }
            ...
            $t1=$this->table_name();
           // $cacheKey = md5($t1.'-'.md5(__CLASS__.__FUNCTION__.json_encode($args) ) );
            $res = array();//cache($cacheKey);
...
//                cache($cacheKey, $res, null, 'arctype');
        ...
        if ($level > 0 && $level < $arctype_max_level)
        {
            ...
            {
                if ($val['level'] >= $end_level)     {  unset($options[$key]);  }
            }
        }
        $pre_key = 0;
        $select = '';
        foreach ($options AS $key => $value)
        {
            ...
            if (($pre_key > 0) && ($options[$pre_key]['id'] == $options[$key]['parent_id'])) {
        ...
        return ($re_type == true) ? $select : $options;
    }
    ...
    public function arctype_options($spec_id, $arr)
    {
        ...
        if (isset($cat_options[$spec_id]))  {  return $cat_options[$spec_id]; }
