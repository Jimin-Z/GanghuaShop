...
class AskLogic extends Model
{
    ...
    public function GetUrlData($param = [], $SpecifyUrl = null)
    {
        ...
        if (!empty($SpecifyUrl)) {
          $result=(!empty($result[$SpecifyUrl])) ? $result[$SpecifyUrl] : $result['NewDateUrl'];
        }
        return $result;
...
EOF;
    ...
    public function GetSeoData($type_id = 0)
    {
        ...
        if ('index' == $inc_type) {
            $s1=$arctypeInfo['seo_title'];
            $seoInfo['seo_title'] = !empty($s1) ? $s1 : $arctypeInfo['typename'];
            $s1=$arctypeInfo['seo_keywords'];
            $seoInfo['seo_keywords'] = !empty($s1) ? $s1 : '';
            $s1=$arctypeInfo['seo_description'];
            $seoInfo['seo_description'] = !empty($s1) ? $s1 : '';
