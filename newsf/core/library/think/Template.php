...
class Template
{
    ...
    public function fetch($template, $vars = [], $config = [])
    {
        ...
        if (is_array($template)) { // 引入模板错误的友好提示 
    ...
    private function compiler(&$content, $cacheFile)
    {
        ...
        if ($this->config['layout_on']) {
            if (false !== strpos($content, '{__NOLAYOUT__}')) {
                ...
                if (is_array($layoutFile)) { // 引入模板的错误友好提示 
    ...
    private function parsePhp(&$content)
    {
        ...
        if ($this->config['tpl_deny_php'] && false !== strpos($content, '<?php')) {
            if (config('app_debug')) { // 调试模式下中断模板渲染 
    ...
    private function parseLayout(&$content)
    {
        ...
        if (preg_match($this->getRegex('layout'), $content, $matches)) {
            ...
            if (!$this->config['layout_on'] || $this->config['layout_name'] != $array['name']) {
                ...
                if (is_array($layoutFile)) { // 引入模板错误的友好提示 
                    $content = !empty($layoutFile['msg']) ? $layoutFile['msg'] : $content;
                } else if ($layoutFile) {
