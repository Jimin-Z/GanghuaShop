...
class Validate
{
    ...
    protected function is($value, $rule, $data = [])
    {
        switch ($rule) {
            ...
            case 'file':
                ...
                break;
            case 'image':
                $result = $value instanceof File && in_array($this->getImageType($value->ey_getFilename()), [1, 2, 3, 6, 15, 18, 8080]); // 
    ...
    protected function image($file, $rule)
    {
        ...
        if ($rule) {
            $rule                        = explode(',', $rule);
            list($width, $height, $type) = getimagesize($file->ey_getFilename()); // 
            ...
            return in_array($this->getImageType($file->ey_getFilename()), [1, 2, 3, 6, 15, 18, 8080]); // 
