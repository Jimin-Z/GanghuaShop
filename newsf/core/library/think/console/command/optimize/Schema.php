...
class Schema extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        if (!is_dir(DATA_PATH . 'schema')) {
            @mkdir(DATA_PATH . 'schema', 0755, true); // 
        }
        $config = [];
    ...
    protected function buildModelSchema($class)
    {
        ...
        if (!$reflect->isAbstract() && $reflect->isSubclassOf('\think\Model')) {
            ...
            $filename = preg_replace('/^'.$prefix.'/i', 'ey_', $table); // 
            file_put_contents(DATA_PATH . 'schema' . DS . $filename . EXT, $content); // 
        }
    }
    ...
    protected function buildDataBaseSchema($tables, $db, $config)
    {
        ...
        foreach ($tables as $table) {
            $content = '<?php ' . PHP_EOL . 'return ';
            $info    = Db::connect($config)->getFields($table); // 
