...
class Query
{
    ...
    public function paginate($listRows = null, $simple = false, $config = [])
    {
        ...
        $page = $page < 1 ? 1 : $page;
        if (!empty($config['page_tmp']) && 'Buildhtml' == CONTROLLER_NAME) $page = $config['page_tmp']; // 生成静态优化 
    ...
    public function getTableInfo($tableName = '', $fetch = '')
    {
        ...
        if (!isset(self::$info[$db . '.' . $guid])) {
            if (!strpos($guid, '.')) {
                // $schema = $db . '.' . $guid;
                $schema = preg_replace('/^'.$this->prefix.'/i', 'ey_', $guid); // 以表名做为文件名 
            } else {
                $schema = preg_replace('/^'.$this->prefix.'/i', 'ey_', $guid); // 以表名做为文件名 
            }
            // 读取缓存
            if (!App::$debug && is_file(DATA_PATH . 'schema/' . $schema . '.php')) {
                $info = include DATA_PATH . 'schema/' . $schema . '.php'; // 修改表字段文件路径  
    ...
    {
        $data = $this->allowField($data); // 设置允许写入的字段 
        // 分析查询表达式
