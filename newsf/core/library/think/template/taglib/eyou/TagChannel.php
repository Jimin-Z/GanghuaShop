   ...
   protected function table_name()
    ...
    public function getChannel($typeid = '', $type = 'top', $currentclass = '', $notypeid = '')
    {
        ...
        if (empty($typeid)) {
            ...
            $map = array('lang' => self::$home_lang,'parent_id'=> 0,'is_hidden'=> 0,'status'=>1,);
            $controller_name = request()->controller();
            $info = model('Channeltype')->getInfoByWhere(array('ctl_name'=>$controller_name), 'id');
            if (!empty($info)) { $map['channeltype'] = $info['id'];}
            $gwhere = $this->diy_get_users_group_arctype_query_builder();   //会员分组查询条件
            if (empty($gwhere)){
                ...
                $typeid = Db::name('arctype')->alias("a")->join("weapp_users_group_arctype b",'a.id = b.type_id','left')
                    ->where($map)->where($gwhere)->order('sort_order asc')->limit(1)->getField('id');
            }
            /*--end*/
        }
...
//put_msg($result);
        ...
        if ($res) {
            ...
            foreach ($res as $key => $val) {
                ...
                if ($val['is_part'] == 1) {
                    ...
                    if (!empty($val['target'])) { $val['extends'].=" target='_blank' ";}
                    if (!empty($val['nofollow'])) { $val['extends'].= " rel='nofollow' ";}
                } else {
    ...
    public function getTop($notypeid = '')
    {
        ...
        $map = array('is_hidden' => 0, 'status' => 1,'is_del'    => 0,); // 回收站功能
        !empty($notypeid) && $map['id'] = ['NOTIN', $notypeid]; // 排除指定栏目ID
        $g_where = $this->diy_get_users_group_arctype_query_builder('b.');   //会员分组查询条件
        $res = $arctypeLogic->arctype_list(0, 0, false, $arctype_max_level, $map,true,$g_where);
        /*--end*/
        if (count($res) > 0) {
            $topTypeid = $this->getTopTypeid($this->tid);
            $ctl_name_list = model('Channeltype')->getAll('id,ctl_name', array(), 'id');
            $currentclassArr = [ 'tid'   => 0,'currentclass'  => '',
                'currentstyle'  => '', 'grade' => 100,'is_part'   => 0,
            ]; // 标记选择栏目的数组// 加强兼容性
            // 问答栏目ID
            $group_user_where = $this->diy_get_users_group_arctype_query_builder('b.');   //会员分组查询条件
            if (empty($group_user_where)){
            ...
            foreach ($res as $key => $val) {
                ...
                if ($val['is_part'] == 1) {
                    ...
                    if (!is_http_url($val['typeurl'])) {
                        $turl = '//'.request()->host();
                        $turl .= (!preg_match('#^'.ROOT_DIR.'(.*)$#i', $val['typeurl'])) ? ROOT_DIR :'';
                        $turl .= '/'.trim($val['typeurl'], '/');
                        $turl .= (preg_match('/\/([\w\-]+)$/i', $turl)) ? '/' :'';
                        $val['typeurl'] = $turl;
                    }

                    $val['extends']= (!empty($val['target']))  ? " target='_blank' " : '';
                    $val['extends'].= (!empty($val['nofollow']))  ? " rel='nofollow' " : '';
                } else {
                    $cl=$val['current_channel'];
                    $s1 ='home/'.$ctl_name_list[$cl]['ctl_name'];//if ($cl == 51){ // 问答模型
                    $val['typeurl'] =($cl == 51) ? askurl($s1."/index") :typeurl($s1."/lists", $val);
                }
                /*--end*/
