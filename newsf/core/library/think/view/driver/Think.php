    ...
    private function parseTemplate($template)
    {
        ...
        if (0 !== strpos($template, '/')) {
            ...
            if (in_array($request->module(), ['admin']) && 'weapp' == strtolower($request->controller()) && 'execute' == strtolower($request->action())) {
                ...
                if ($controller) {
                    if ('user' == $request->module()) { // 会员中心模板切换，兼容响应式模板目录下，users目录可以灵活定义wap手机端会员中心模板 
                        ...
                        if (preg_match('/\/ask\/([a-zA-Z0-9_-]+)\//i', $template_tmp)) { // 问答中心模板切换，兼容响应式模板目录下，ask目录可以灵活定义wap手机端问答中心模板 
                            $users_wap_tpl_dir = config('ey_config.users_wap_tpl_dir');
        ...
        return $path . ltrim($template, $depr) . '.' . ltrim($this->config['view_suffix'], '.'); // 过滤多余的斜杆 
