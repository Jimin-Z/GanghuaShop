<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:30:"./template/pc/view_article.htm";i:1728638483;s:62:"D:\wampserver\wamp64\www\hkshop\newsf\template\pc\headerym.htm";i:1729046592;s:60:"D:\wampserver\wamp64\www\hkshop\newsf\template\pc\footer.htm";i:1728737211;}*/ ?>
...
<font size="4">您的位置: <?php  $typeid = ""; if(empty($typeid) && isset($channelartlist["id"]) && !empty($channelartlist["id"])) : $typeid = intval($channelartlist["id"]); endif;  $tagPosition = new \think\template\taglib\eyou\TagPosition; $__VALUE__ = $tagPosition->getPosition($typeid, "", ""); echo $__VALUE__; ?></font> </div>
<div class="container">
  <div class="con mt10 white_bg mb30">
    <div class="text-center">
      <h1 ><font size=6><?php echo $eyou['field']['title']; ?></font></h1><hr>
      <font size=4><time><?php echo MyDate('Y/m/d',$eyou['field']['update_time']); ?></time></font>
      <hr>
    </div>
    <div> 
     
<?php 
 if(!empty($eyou['field']['subtitle'])){
   include ($eyou['field']['subtitle']);
 } else{
   echo $eyou['field']['content']; 
 }
 ?>

     </div>
<?php 
       $s0=$eyou['field']['shipin'];
        if(!empty($s0)){
        $s1='   <div style="background: white;">';
        $s1.=' <div class="video"><video style="margin: 0 5%" width="90%" height="360" controls="controls" loop="loop">';
        $s1.='<source src="';
        $s1.=$s0.'" type="video/mp4">Your browser does not support the video tag. </video></div></div>';
     echo  $s1;
    }
   ?>

    <div class="pb50 gray38"> <?php  $tagPrenext = new \think\template\taglib\eyou\TagPrenext; $_result = $tagPrenext->getPrenext("pre");if(!empty($_result) || (($_result instanceof \think\Collection || $_result instanceof \think\Paginator ) && $_result->isEmpty())): $field = $_result; $field["title"] = text_msubstr($field["title"], 0, 30, false); ?>
      <div><?php  $tagLang = new \think\template\taglib\eyou\TagLang; $__VALUE__ = $tagLang->getLang('a:1:{s:4:"name";s:5:"sys11";}'); echo $__VALUE__; ?>：<a href='<?php echo $field['arcurl']; ?>'><?php echo $field['title']; ?></a></div>
      <?php else: ?>
      <p class="pb10"><?php  $tagLang = new \think\template\taglib\eyou\TagLang; $__VALUE__ = $tagLang->getLang('a:1:{s:4:"name";s:5:"sys11";}'); echo $__VALUE__; ?>：<a><?php  $tagLang = new \think\template\taglib\eyou\TagLang; $__VALUE__ = $tagLang->getLang('a:1:{s:4:"name";s:5:"sys10";}'); echo $__VALUE__; ?></a></p>
      <?php endif; $field = [];  $tagPrenext = new \think\template\taglib\eyou\TagPrenext; $_result = $tagPrenext->getPrenext("next");if(!empty($_result) || (($_result instanceof \think\Collection || $_result instanceof \think\Paginator ) && $_result->isEmpty())): $field = $_result; $field["title"] = text_msubstr($field["title"], 0, 30, false); ?>
      <p><?php  $tagLang = new \think\template\taglib\eyou\TagLang; $__VALUE__ = $tagLang->getLang('a:1:{s:4:"name";s:5:"sys12";}'); echo $__VALUE__; ?>：<a href='<?php echo $field['arcurl']; ?>'><?php echo $field['title']; ?></a></p>
      <?php else: ?>
      <p><?php  $tagLang = new \think\template\taglib\eyou\TagLang; $__VALUE__ = $tagLang->getLang('a:1:{s:4:"name";s:5:"sys12";}'); echo $__VALUE__; ?>：<a><?php  $tagLang = new \think\template\taglib\eyou\TagLang; $__VALUE__ = $tagLang->getLang('a:1:{s:4:"name";s:5:"sys10";}'); echo $__VALUE__; ?></a></p>
      <?php endif; $field = []; ?> </div>
  </div>
</div>
<!--_footer 作为公共模版分离出去-->
