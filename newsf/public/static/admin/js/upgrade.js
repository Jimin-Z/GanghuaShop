...
function btn_upgrade(obj, type)
{
    ...
    if (filelist_arr.length > 50) {
        ...
        filelist_arr.push("……");
        filelist_arr.push("<a href='https://www.hkshop.com/plus/upgrade.php?version="+curent_version+"-"+version+"' target='_blank'>此次更新涉及的全部文件，点击这里查看！</a>");
            ...
            if (1 == res.code) {
                ...
                if (2 == res.data.code) { 
                    var alert = parent.layer.alert(res.msg, {icon: 2, title:false, btn: ['立即查看']}, function(){
                        window.parent.open('https://www.hkshop.com/plus/view.php?aid=9105');
                    });
                } else {
